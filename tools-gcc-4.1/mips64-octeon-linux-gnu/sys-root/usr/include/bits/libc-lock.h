...
# define __libc_lock_lock(NAME) \
  ({ lll_lock (NAME); 0; })
# else
/* TLS is expensive on MIPS.  Only do this with multi-threading.  */
#  define __libc_lock_lock(NAME) \
  ({ if (__pthread_mutex_lock) { lll_lock (NAME); } 0; })
# endif
#else
# define __libc_lock_lock(NAME) \
...
#  define __libc_lock_unlock_recursive(NAME) \
  do {									      \
    if (__pthread_mutex_unlock && --(NAME).cnt == 0)			      \
      {									      \
	(NAME).owner = NULL;						      \
	lll_unlock ((NAME).lock);					      \
      }									      \
