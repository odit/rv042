 ...
 *
 * $Id: sch_htb.c 1196 2007-11-07 02:51:06Z richie $
 */
...
#define HTB_DEBUG 	/* compile debugging support (activated by tc tool) */
...
//richie add 060213: to get device lan ip
...
static struct htb_class *htb_classify(struct sk_buff *skb, struct Qdisc *sch, int *qerr)
{
	...
	*qerr = NET_XMIT_BYPASS;
	...
	while (tcf && (result = tc_classify(skb, tcf, &res)) >= 0) 
	{
		...
		case TC_ACT_QUEUED:
		case TC_ACT_STOLEN: 
			printk(KERN_EMERG "*qerr = NET_XMIT_SUCCESS;\n");
			*qerr = NET_XMIT_SUCCESS;
		case TC_ACT_SHOT:
			printk(KERN_EMERG "return NULL;\n");
			return NULL;
		}
#elif defined(CONFIG_NET_CLS_POLICE)
		if (result == TC_POLICE_SHOT)
		{
			printk(KERN_EMERG "return HTB_DIRECT;\n");
			return HTB_DIRECT;
		}
#endif
...
static int htb_init(struct Qdisc *sch, struct rtattr *opt)
{
	...
	//richie add to get device lan ip  (2006-02-13)
	struct net_device *device=NULL;
	struct in_device *in_dev = NULL;
	struct in_ifaddr *ifa_list = NULL;


	printk("\n\n=====htb_init start time j=%ld======\n\n",jiffies);
...
//read_unlock_bh(&dev_base_lock);
	printk("device_ip  111 = %x\n", device_ip);


#ifdef HTB_DEBUG
	printk(KERN_EMERG "HTB init, kernel part version %d.%d\n",
			  HTB_VER >> 16,HTB_VER & 0xffff);
#endif
	if (!opt || rtattr_parse_nested(tb, TCA_HTB_INIT, opt) ||
			tb[TCA_HTB_INIT-1] == NULL ||
			RTA_PAYLOAD(tb[TCA_HTB_INIT-1]) < sizeof(*gopt)) {
		printk(KERN_EMERG "HTB: hey probably you have bad tc tool ?\n");
		return -EINVAL;
	}
	gopt = RTA_DATA(tb[TCA_HTB_INIT-1]);
	if (gopt->version != HTB_VER >> 16) {
		printk(KERN_EMERG "HTB: need tc/htb version %d (minor is %d), you have %d\n",
		...
		u32 parentid, struct rtattr **tca, unsigned long *arg)
{
	...
	if (!cl) { /* new class */
		...
		if (parent && parent->parent && parent->parent->level < 2) {
			printk(KERN_EMERG "htb: tree is too deep\n");
			goto failure;
		}
		err = -ENOBUFS;
	...
	if (!cl->level) {
		...
		if (!hopt->quantum && cl->un.leaf.quantum < 1000) {
			//printk(KERN_EMERG "HTB: quantum of class %X is small. Consider r2q change.\n", cl->classid);
			cl->un.leaf.quantum = 1000;
		}
		if (!hopt->quantum && cl->un.leaf.quantum > 200000) {
			//printk(KERN_EMERG "HTB: quantum of class %X is big. Consider r2q change.\n", cl->classid);
