...
//max add for connect on demand
//max add for usb-key show wrong log ,2008/03/04
#define SA_LIST_NAME_LEN 50
//<<
struct is_sa_list
{
	char name[SA_LIST_NAME_LEN];
	int  status; //0:not use 1:used  2: using 
        struct in_addr  L_start;
        struct in_addr  L_end;
        struct in_addr  R_start;
        struct in_addr  R_end;
};
struct _is_sa_list_db
{
	struct is_sa_list  sa_list[400]; 
        //?? need check 
        int lastCounter;
};
static struct _is_sa_list_db is_sa_list_db;
static struct sock *xfrm_nl_sa;
//<<
...
//max add for connect on demand
int __xfrm_packet_check(struct sk_buff *skb, unsigned short family)
{
	int i;
	mm_segment_t 	old_fs;
	loff_t 		pos = 0;
	...
	for(i=0;i<is_sa_list_db.lastCounter;i++)
	{
		if( 
		(skb->nh.iph->daddr  >= is_sa_list_db.sa_list[i].R_start.s_addr) &&
		(skb->nh.iph->daddr  <= is_sa_list_db.sa_list[i].R_end.s_addr) &&
		(skb->nh.iph->saddr  >= is_sa_list_db.sa_list[i].L_start.s_addr) &&	
		//(skb->nh.iph->saddr  >= is_sa_list_db.sa_list[i].L_start.s_addr) )
		(skb->nh.iph->saddr  <= is_sa_list_db.sa_list[i].L_end.s_addr) )//20100409 max: COD crash	
		{
			
			if(is_sa_list_db.sa_list[i].status == 1)
			{
				// this entry had noticed nk_ipsecd, not need to do again.
				return 1; //build connection   		
			}
			//20100409 max: COD crash
			if(smp_processor_id()==1)
			{
				return 1; //build connection   		
			}
			filp = filp_open( "/var/nk_ipsecd.fifo", O_RDWR | O_CREAT | O_TRUNC, 0);
			if (!filp->f_op->write)
			{
				printk("\n MAX MAX : make connection not successful\n");
	                        break;
			}
			old_fs = get_fs();
			set_fs(get_ds());
			// CMD = 3  is "IPSEC_IPC_INIT_CONN_BYNAME"
			sprintf(cfgbuf,"CMD=3&NAME=%s\n",is_sa_list_db.sa_list[i].name);

			ret = filp->f_op->write(filp, (void *)cfgbuf, strlen(cfgbuf), &pos);

			set_fs(old_fs);
			filp_close(filp, NULL);
			is_sa_list_db.sa_list[i].status = 1;
			return 1; //build connection 
...
EXPORT_SYMBOL(__xfrm_packet_check);


static void xfrm_netlink_sa_rcv(struct sock *sk, int len)
{
	int ii;
	...
	do
	{
		...
		while((skb = skb_dequeue(&sk->sk_receive_queue)) != NULL)
		{
			...
			//printk("\n MAX MAX :05 while Enter xfrm_netlink_sa_rcv skb->len=%d  sizeof(struct nlmsghdr)=%d",skb->len,sizeof(struct nlmsghdr));
			//printk("\n MAX MAX :05 while Enter xfrm_netlink_sa_rcv\n");


			if(skb->len >= sizeof(struct nlmsghdr))
			{
				nlh = (struct nlmsghdr *)skb->data;
			        //printk("\n MAX MAX :06 if Enter xfrm_netlink_sa_rcv nlh->nlmsg_len=%d  nskb->len=%d",nlh->nlmsg_len,skb->len);
			        //printk("\n MAX MAX :06 if Enter xfrm_netlink_sa_rcv\n");

				if((nlh->nlmsg_len >= sizeof(struct nlmsghdr))&& (skb->len >= nlh->nlmsg_len))
				{
					//printk("\n MAX MAX :07 Enter xfrm_netlink_sa_rcv status[%d]",((struct  is_sa_list*)NLMSG_DATA(nlh))->status);
					//printk("\n MAX MAX :07 Enter xfrm_netlink_sa_rcv status[%d]\n",((struct  is_sa_list*)NLMSG_DATA(nlh))->status);
					if(((struct  is_sa_list*)NLMSG_DATA(nlh))->status == 0) //0: add 1:delete
					{
                                            memcpy((void *)&is_sa_list_db.sa_list[is_sa_list_db.lastCounter],NLMSG_DATA(nlh),sizeof(struct is_sa_list));				
					    //printk("\n MAX MAX : name:%s %x %x %x %x",is_sa_list_db.sa_list[is_sa_list_db.lastCounter].name,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].R_start.s_addr,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].R_end.s_addr,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].L_start.s_addr,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].L_end.s_addr);
					    //printk("\n MAX MAX : name:%s %x %x %x %x \n",is_sa_list_db.sa_list[is_sa_list_db.lastCounter].name,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].R_start.s_addr,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].R_end.s_addr,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].L_start.s_addr,is_sa_list_db.sa_list[is_sa_list_db.lastCounter].L_end.s_addr);
					    is_sa_list_db.lastCounter += 1;
					}
					else //delete
					{
						for(ii = 0; ii <is_sa_list_db.lastCounter ; ii++)
						{
					             //printk("\n MAX MAX : name [%s][%s]",((struct  is_sa_list*)NLMSG_DATA(nlh))->name,is_sa_list_db.sa_list[ii].name);

							if(!strcmp(((struct  is_sa_list*)NLMSG_DATA(nlh))->name,is_sa_list_db.sa_list[ii].name))
							{
								is_sa_list_db.lastCounter -=1;
					
								if(is_sa_list_db.lastCounter > ii)
								{
								memcpy((void *)&is_sa_list_db.sa_list[ii],(void *)&is_sa_list_db.sa_list[is_sa_list_db.lastCounter],sizeof(struct is_sa_list));
								}
								memset((void *)&is_sa_list_db.sa_list[is_sa_list_db.lastCounter],0,sizeof(struct is_sa_list));		
								break;
							}
						}
					}
	...
	}while(xfrm_nl_sa && xfrm_nl_sa->sk_receive_queue.qlen);
}
//<<
...
//max add for connect on demand
        is_sa_list_db.lastCounter = 0;
	xfrm_nl_sa = netlink_kernel_create(NETLINK_XFRM_SA, 0,
	                                xfrm_netlink_sa_rcv, THIS_MODULE);
