...
DEFINE_RWLOCK(ip_trigger_lock);

//#define ASSERT_READ_LOCK(x) MUST_BE_READ_LOCKED(&ip_trigger_lock)
//#define ASSERT_WRITE_LOCK(x) MUST_BE_WRITE_LOCKED(&ip_trigger_lock)
...
static void __del_trigger(struct ipt_trigger *trig)
{
    DEBUGP("%s: \n", __FUNCTION__);
    IP_NF_ASSERT(trig);
    //MUST_BE_WRITE_LOCKED(&ip_trigger_lock);
