...
//printk("%s exp->dst.port=%u\n", __func__, exp->tuple.dst.u.tcp.port);
//*********************************************************************
#if 1
#if 0
printk("*** %s: proto[%u]: sip[%u.%u.%u.%u]:%u->dip[%u.%u.%u.%u]:%u dev[%s], idev[%s], odev[%s]\n",
	__func__, exp->tuple.dst.protonum, NIPQUAD(ct->tuplehash[!dir].tuple.src.ip),
	exp->tuple.src.u.tcp.port, NIPQUAD(ct->tuplehash[!dir].tuple.dst.ip),
	exp->tuple.dst.u.tcp.port, (*pskb)->dev->name, (*pskb)->input_dev->name,
	(*pskb)->odev->name);
#endif
/*Add fire_spec*/
//printk("ftptype=%d\n", search[i].ftptype);
...
if((*pskb)->input_dev)
{
	//printk(KERN_EMERG "Add fire_spec dev=%s\n", (*pskb)->input_dev->name);
	if(strcmp((*pskb)->input_dev->name, "eth0"))
	{
		...
		/* support Multi WAN */
	//	printk(KERN_EMERG "support Multi WAN \n");
		fire_spec_output_ptr = kmalloc(sizeof(struct fire_spec_output), GFP_ATOMIC);

		if(fire_spec_output_ptr)
		{
// 			fire_spec_output_ptr->saddr = ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.ip;
// 			fire_spec_output_ptr->daddr = ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.ip; 
			if(search[i].ftptype==1) //pasv mode
			{
				fire_spec_output_ptr->saddr = ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.ip;
				fire_spec_output_ptr->daddr = ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.ip; 
				fire_spec_output_ptr->protocol_type = htons(IPPROTO_TCP);
				fire_spec_output_ptr->src_port = 0;
				fire_spec_output_ptr->dst_port = htons(array[4] << 8 | array[5]);
			}
			else if(search[i].ftptype==0) //port mode
			{
				fire_spec_output_ptr->saddr = ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.ip;
				fire_spec_output_ptr->daddr = ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.ip; 
				fire_spec_output_ptr->protocol_type = htons(IPPROTO_TCP);
 				fire_spec_output_ptr->src_port = ntohs(ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all) - 1;
	 			fire_spec_output_ptr->dst_port = 0;
			}
			sscanf((*pskb)->input_dev->name, "eth%d", &wannum);
			/* purpose     : 0013279    author : paul.chen    date : 2010-07-22 */
			/* description : ftp connect crash when WAN device is not ethx           */
			if(wannum>0)
			{
				fdev = __dev_get_by_name(wan_interface_session[wannum-1].name);
			}
			else
			{
				fdev = __dev_get_by_name((*pskb)->input_dev->name);
			}
			//printk("fdev = %s\n", fdev->name);
			fire_spec_output_ptr->dev = fdev;
			fire_spec_output_ptr->age = 30;
			...
			//printk(KERN_EMERG "*** %s: fire_spec_output_ptr alloc success, proto[%u]: sip[%u.%u.%u.%u]:%u->dip[%u.%u.%u.%u]:%u dev[%s], idev[%s], odev[%s]\n",
