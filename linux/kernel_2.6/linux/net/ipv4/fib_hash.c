 ...
 *
 * Version:	$Id: fib_hash.c 412 2007-05-23 06:36:56Z tren $
 *
...
/*2006/11/1 trenchen : support specify out device*/
int
nk_fn_hash_lookup(struct fib_table *tb, const struct flowi *flp, struct fib_result *res, struct net_device *out_device)
{
	int err;
	struct fn_zone *fz;
	...
	for (fz = t->fn_zone_list; fz; fz = fz->fz_next) {
		...
		hlist_for_each_entry(f, node, head, fn_hash) {
			/*2007/5/23 trenchen : kernel 2.6 use different data strcut to resprent different routing entries with the same subnet but different out interface*/
//			struct fib_alias *fa;
//			fa=list_entry( (&f->fn_alias)->next, typeof(*fa), fa_list);
//			if (f->fn_key != k || (out_device && ( (fa->fa_info->fib_nhs > 1) || strcmp(out_device->name,fa->fa_info->fib_dev->name))) ) {
			if (f->fn_key != k)
				continue;
#if 0
			err = fib_semantic_match(&f->fn_alias,
						 ...
						 fz->fz_order);
#endif
			err = nk_fib_semantic_match(&f->fn_alias,
						 flp, res,
						 f->fn_key, fz->fz_mask,
						 fz->fz_order,out_device);
