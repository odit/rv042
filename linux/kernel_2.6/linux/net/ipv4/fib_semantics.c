 ...
 *
 * Version:	$Id: fib_semantics.c 2387 2009-08-17 07:44:04Z richie $
 *
...
/* support IP balance enhancement: weight(WEIGHT_HIGH_PRIORITY) has the high priority  -- incifer 2008/11 */
#ifdef CONFIG_IPBALANCE_ENHANCE
/**
	Distribute IP to the WAN with WEIGHT_HIGH_PRIORITY
	Declare at file: webBoot.c, autobalance.c, fib_semantics.c
**/
#define WEIGHT_HIGH_PRIORITY	255
#endif
...
/*2007/5/23 trenchen : copy from fib_semantic_match and modify for source route*/
int nk_fib_semantic_match(struct list_head *head, const struct flowi *flp,
		       struct fib_result *res, __u32 zone, __u32 mask, 
			int prefixlen, struct net_device *out_device)
{
	struct fib_alias *fa;
	int nh_sel = 0;
	...
	list_for_each_entry_rcu(fa, head, fa_list) {
		...
		/*2007/5/23 trenchen : only the fa that it's interface match the out_device is what we need*/ 
		if( out_device && ( (fa->fa_info->fib_nhs > 1) || strcmp(out_device->name,fa->fa_info->fib_dev->name) ) )
			continue;
...
#ifdef CONFIG_IPBALANCE_ENHANCE
	int weight_num=0;
	int nh_weight_high_sel[CONFIG_NK_NUM_WAN+CONFIG_NK_NUM_DMZ];
#endif
...
#ifdef CONFIG_IPBALANCE_ENHANCE
/**
	if weight == WEIGHT_HIGH_PRIORITY, we select the wan first
	1. find how many route have weight(255), and record the nhsel
	2. select multipath from route with weight(255)
**/
	/* 1. find how many route have weight(255), and record the nhsel */
	change_nexthops(fi){
		if (!(nh->nh_flags&RTNH_F_DEAD)) {
			if(nh->nh_weight == WEIGHT_HIGH_PRIORITY)
			{
				nh_weight_high_sel[weight_num] = nhsel;
				weight_num++;
			}
		}
	}endfor_nexthops(fi);
	/* 2. select multipath from route with weight(WEIGHT_HIGH_PRIORITY) */
	if(weight_num)
	{
		fi->fib_power = 0;/* reset it */
		w = jiffies % weight_num;
		res->nh_sel = nh_weight_high_sel[w];
		spin_unlock_bh(&fib_multipath_lock);
		return;
	}
#endif
