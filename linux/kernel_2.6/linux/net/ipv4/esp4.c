...
static int esp_output(struct xfrm_state *x, struct sk_buff *skb)
{
	...
	*(u8*)(trailer->tail - 1) = top_iph->protocol;
	...
	{
		int ret;
		__skb_pull(skb, (unsigned char *)esph - skb->nh.raw);
		if (esp->conf.ivlen) {
			memcpy(esph->enc_data, esp->conf.ivec, crypto_tfm_alg_ivsize(tfm));
		}
		ret = cavium_process_esp_pkt(esp, skb);
		__skb_push(skb, (unsigned char *)esph - skb->nh.raw);
		if (esp->conf.ivlen) {
	...
	/*	
		if (esp->conf.ivlen)
			crypto_cipher_set_iv(tfm, esp->conf.ivec, crypto_tfm_alg_ivsize(tfm));
	 */
	   /* Ideally todo(right now this is not done): Unsupported ciphers in S/w */
	 	goto error;
	}
...
static int esp_input(struct xfrm_state *x, struct xfrm_decap_state *decap, struct sk_buff *skb)
{
	...
	#if defined (CONFIG_OCTEON_NATIVE_IPSEC)
	{
		int ret = cavium_process_esp_pkt(esp, skb);
		if (unlikely(ret)) {
			x->stats.integrity_failed++;
			goto out;
		}
...
static int __init esp4_init(void)
{
	...
	printk(KERN_INFO "Octeon Native IPsec hw acceralation actived..\n");
#endif
