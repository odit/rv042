...
int broadcast_groups = 0x7fff;
module_param(broadcast_groups, int, 0644);
MODULE_PARM_DESC(broadcast_groups,
                 " Bitmask of groups to send broadcasts to. Defaults to 0xffff.\n"
...
{
    return phys_to_virt(((packet_ptr.s.addr >> 7) - packet_ptr.s.back) << 7);
}
...
{
    uint64_t address = octeon_read_csr(OCTEON_FPA_ALLOC(pool));
    if (address)
...
{
    wmb();
    octeon_write_csr(OCTEON_FPA_FREE(pool, virt_to_phys(ptr)), num_cache_lines);
}
 ...
 */
static inline void pow_work_submit(octeon_wqe_t *wqp, uint32_t tag, uint64_t tag_type, uint64_t qos, uint64_t grp)
{
    uint64_t tag_req = (4ull<<44) | (qos<<39) | (grp<<35) | (tag_type<<32) | tag;
    wmb();
    octeon_write_csr(OCTEON_POW_WORK_SUBMIT(virt_to_phys(wqp)), tag_req);
}
...
{
    int64_t result = octeon_read_csr(OCTEON_POW_WORK_REQUEST(wait));
    if (result < 0)
        return NULL;
    else
...
static int free_work(octeon_wqe_t *work)
{
    ...
    while (segments--)
    {
        octeon_buf_ptr_t next_ptr = *(octeon_buf_ptr_t*)phys_to_virt(segment_ptr.s.addr-8);
        if (unlikely(!segment_ptr.s.i))
            fpa_free(get_buffer_ptr(segment_ptr), segment_ptr.s.pool, 0);
...
static int packet_transmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    /* Broadcast and multicasts go to all groups in the module param
        broadcast_groups. Unicast use the low order dest mac byte as the group
        number */
    if ((*(uint64_t*)(skb->data)>>16) & 0x010000000000ul)
        send_group_mask = broadcast_groups;
    else
        send_group_mask = 1<<(skb->data[5] & (NUM_GROUPS-1));
    send_group_mask &= ~(1<<receive_group);

    /* It is ugly, but we need to send multiple times for broadcast packets.
        The hardware doesn't support submitting work to multiple groups */
    for (send_group=0; send_group<NUM_GROUPS; send_group++)
    {
        ...
        /* Calculate where we need to copy the data to. We need to leave 8 bytes
            for a next pointer (unused). Then we need to align the IP packet src
        ...
        if (unlikely(copy_location + skb->len > packet_buffer + fpa_packet_pool_size))
        {
        ...
        pow_work_submit(work, work->tag, work->tag_type, work->qos, work->grp);
        work = NULL;
        packet_buffer = NULL;
    }
 ...
 * @return
 */
static irqreturn_t do_interrupt(int cpl, void *dev_id, struct pt_regs *regs)
{
    ...
    octeon_write_csr(OCTEON_POW_WQ_INT, 0x10001<<receive_group);

    /* Only allow work for our group */
    old_group_mask = octeon_read_csr(OCTEON_POW_PP_GRP_MSKX(coreid));
    octeon_write_csr(OCTEON_POW_PP_GRP_MSKX(coreid), 1<<receive_group);
    ...
    while (1)
    {
        ...
        else
        {
            ...
            while (segments--)
            {
                ...
                int segment_size = fpa_packet_pool_size - (segment_ptr.s.addr - (((segment_ptr.s.addr >> 7) - segment_ptr.s.back) << 7));
                /* Don't copy more than what is left in the packet */
                if (segment_size > len)
                    segment_size = len;
                /* Copy the data into the packet */
                memcpy(skb_put(skb, segment_size), phys_to_virt(segment_ptr.s.addr), segment_size);
    ...
    octeon_write_csr(OCTEON_POW_PP_GRP_MSKX(coreid), old_group_mask);
    return IRQ_HANDLED;
}
...
static int device_init(struct net_device *dev)
{
    ...
    dev->dev_addr[3]        = 0;
    dev->dev_addr[4]        = 0;
    dev->dev_addr[5]        = octeon_get_core_num();
    return 0;
}
...
static int __init ethernet_pow_init(void)
{
    ...
    {
        printk("Invalid receive group. Must be 0-%d\n", NUM_GROUPS-1);
        return -1;
    }

    if ((broadcast_groups & ((1<<NUM_GROUPS)-1)) != broadcast_groups)
    {
        printk("Invalid broadcast group mask.\n");
        return -1;
    }
    ...
    if (receive_group < 0)
        receive_group = octeon_get_core_num();
    ...
    {
        printk("Failed to allocate ethernet device\n");
        return -1;
    }
    ...
    while ((octeon_read_csr(OCTEON_IPD_CTL_STATUS) & 1) == 0)
    {
        mdelay(100);
    }
    printk("Done\n");

    /* Read the configured size of the FPA packet buffers. This way we don't
        need changes if someone chooses to use a different buffer size */
    fpa_packet_pool_size = (octeon_read_csr(OCTEON_IPD_PACKET_MBUFF_SIZE) & 0xfff) * 8;

    /* Read the work queue pool */
    fpa_wqe_pool = octeon_read_csr(OCTEON_IPD_WQE_FPA_QUEUE) & 7;

    if (register_netdev(global_device)<0)
    {
        printk("Failed to register ethernet device\n");
        kfree(global_device);
        return -1;
    }

    /* Register an IRQ hander for to receive POW interrupts */
    request_irq(8 + receive_group, do_interrupt, SA_SHIRQ, "POW Ethernet", global_device);

    /* Enable POW interrupt when our port has at least one packet */
    octeon_write_csr(OCTEON_POW_WQ_INT_THRX(receive_group), 0x1001);
    return 0;
}
...
static void __exit ethernet_pow_cleanup(void)
{
    ...
    octeon_write_csr(OCTEON_POW_WQ_INT_THRX(receive_group), 0);
