...
static inline void
OUTW_swap(unsigned short value, unsigned short *addr)
{
  // swap
...
static inline void
OUTW(unsigned short value, unsigned short *addr)
{
  // no swap
  *(volatile unsigned char *)(addr) = value>>8;
  *(volatile unsigned char *)(addr) = value;
...
#else
	OUTW_swap((unsigned short)portno, (unsigned short *)(dev->base_addr + ADD_PORT));
...
#else
	OUTW_swap((unsigned short)portno, (unsigned short *)(dev->base_addr + ADD_PORT));
	OUTW_swap((unsigned short)value, (unsigned short *)(dev->base_addr + DATA_PORT));
#endif
}
...
#if 1 // kihbingo
static void
readstring(struct net_device *dev, int portno, void *Ptr, int len)
{
...
#else
	OUTW_swap((unsigned short)value, (unsigned short *)(dev->base_addr + portno));
#endif
}
#if 1 // kihbingo
static void
writestring(struct net_device *dev, int portno, void *Ptr, int len)
{
  unsigned short *ptr = (unsigned short *)Ptr;

  // no swap
  while (len > 0) {
    OUTW((unsigned short)*ptr, (unsigned short *)(dev->base_addr + portno));
    ptr++;
    len--;
  }
}
  ...
  tmp = readreg(dev, PP_ChipID);
	if (tmp != CHIP_EISA_ID_SIG) {
		printk(KERN_INFO "%s: incorrect signature at %x: 0x%x!="
			CHIP_EISA_ID_SIG_STR "\n",
			dev->name, (unsigned int)(dev->base_addr + DATA_PORT), tmp);
  		retval = -ENODEV;
  		goto out2;
	}
#endif
