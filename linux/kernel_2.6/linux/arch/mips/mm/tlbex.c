		...
		unsigned int tmp, unsigned int pmd)
{
	...
	i_SRL(p, tmp, tmp, 6); /* convert to entrylo0 */
	i_mtc0(p, tmp, C0_ENTRYLO0); /* load it */
	i_ADDIU(p, tmp, tmp, (HPAGE_SIZE >> 1) >> 6); /* convert to entrylo1 */
...
static void __init build_r4000_tlb_refill_handler(void)
{
	...
	i_ld(&p, K0, FAST_ACCESS_THREAD_OFFSET, 0);  /* K0 = thread pointer */
#endif
	...
	if ((p - tlb_handler) <= 32) {
		...
		/* If the split is on an eret, don't insert a branch */
		if (*split == 0x42000018 /* ERET */)
		{
	...
	set_handler(0x0, final_handler, 0x100);
}
...
static void __init build_r4000_tlb_load_handler(void)
{
	...
	build_tlb_probe_entry(&p);
	il_bbit0(&p, &r, PTE, __ilog2(_PAGE_READ), label_nopage_tlbs);
	...
	i_SW(&p, PTE, 0, PTR);
#endif
	i_SRL(&p, PTE, PTE, 6); /* convert to entrylo0 */
