...
static int sil24_issue_SRST(struct ata_port *ap)
{
	...
	msleep(50);	/* Add delay, or the reset command will fail */
#endif
...
static int sil24_port_start(struct ata_port *ap)
{
	...
	/* Even though the device support 64bit addresses for disk transfers,
	    the data structures used for list only support 32bit addresses */
	pci_set_dma_mask(to_pci_dev(dev), DMA_32BIT_MASK);
	cb = dma_alloc_coherent(dev, cb_size, &cb_dma, GFP_KERNEL);
	pci_set_dma_mask(to_pci_dev(dev), DMA_64BIT_MASK);
	if (!cb)
		goto err_out_pp;
	memset(cb, 0, cb_size);
...
#ifdef CONFIG_CPU_CAVIUM_OCTEON
	/* Hitachi Deskstar drives take forever to spinup and be ready for
		probing. I've seen instances where 4 seconds isn't long enough.
		Hopefully this will always be long enough. If it isn't long
		enough, you receive continuous interrupts from a disabled
		channel */
	msleep(7000);
#endif
