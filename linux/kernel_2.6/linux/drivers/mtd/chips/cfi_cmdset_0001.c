 ...
 *
 * $Id: cfi_cmdset_0001.c 5045 2010-05-14 06:10:19Z david.kao $
 *
 *
...
read_pri_intelext(struct map_info *map, __u16 adr)
{
 ...
 again:
	...
	/*if (extp->MajorVersion != '1' ||
	    (extp->MinorVersion < '0' || extp->MinorVersion > '9')) 
	{
				    ...
				    unsigned long *pvec_seek, int len)
{
	...
	*pvec_seek = vec_seek;
	...
	/* check for errors */
	//2009/12/9 trenchen : bug fix, intel new flash(P33BF) will get error status, ignore it
#if 1
	if (map_word_bitsset(map, status, CMD(0x1a))) {
		unsigned long chipstatus = MERGESTATUS(status);
