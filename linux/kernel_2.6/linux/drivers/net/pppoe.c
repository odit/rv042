...
static int __pppoe_xmit(struct sock *sk, struct sk_buff *skb)
{
	...
	skb2->alg = skb->alg;
	skb2->qos_need = skb->qos_need;
	skb2->qos_dev = skb->qos_dev;     //2008/3/17 trenchen : support QoS nat Acc
	skb2->qos_cook = skb->qos_cook;
	skb2->conntrack_ptr = skb->conntrack_ptr;
