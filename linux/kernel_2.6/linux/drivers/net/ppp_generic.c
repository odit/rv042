...
ppp_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	//printk(KERN_EMERG "ppp_start_xmit skb[%p]----------------------------------\n", skb);
...
ppp_xmit_process(struct ppp *ppp)
{
	...
	if (ppp->dev != 0) {
		//if(ppp->xmit_pending)
		//	printk(KERN_EMERG "ppp_xmit_process ppp->xmit_pending != 0 111 ********\n");
		ppp_push(ppp);
		//if(ppp->xmit_pending)
		//	printk(KERN_EMERG "ppp_xmit_process ppp->xmit_pending != 0 222 ********\n");
		while (ppp->xmit_pending == 0
			&& (skb = skb_dequeue(&ppp->file.xq)) != 0)
		{
		//	printk(KERN_EMERG "ppp_xmit_process skb[%p]\n", skb);
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	//printk(KERN_EMERG "ppp_send_frame skb[%p]\n", skb);
...
static void hex_dump(void *buf, int len)
{
	...
	for (i = 0 ; i < len; i++)
	{
		printk(KERN_EMERG "%02x", ((uint8_t*)buf)[i]);
		if (i%2)
			printk(" ");
		if (15 == i%16)
			printk(KERN_EMERG "\n");
...
{
	printk(KERN_EMERG "SKB dump skb=%p,head=%p, data=%p, len=%d\n",skb, skb->head, skb->data, skb->len);
...
ppp_push(struct ppp *ppp)
{
	...
	//printk(KERN_EMERG "ppp_push skb[%p]\n", skb);
	if (skb == 0)
		return;
	...
	if ((ppp->flags & SC_MULTILINK) == 0) {
		...
		if (pch->chan) {
			//printk(KERN_EMERG "start_xmit, pch[%p] chan[%p] start_xmit[%p]==============================\n", pch, pch->chan, pch->chan->ops->start_xmit);
			//printk(KERN_EMERG "ppp_push start_xmit, pch[%p] chan[%p] start_xmit[%p]==============================\n", pch, pch->chan, pch->chan->ops->start_xmit);
			//skb_dump(skb);
			...
			else //transmit fail
			{
				...
				if(ppp->xc_state)
				{
					struct ppp_mppe_state *statep = ppp->xc_state;
					struct timer_list *timer = &(statep->ppp_mppe_block_timer);
...
ppp_channel_push(struct channel *pch)
{
	...
	if (skb_queue_empty(&pch->file.xq)) {
		...
		if (ppp != 0)
		{
			//printk(KERN_EMERG "ppp_channel_push call ppp_xmit_process******\n");
			ppp_xmit_process(ppp);
		}
...
ppp_set_compress(struct ppp *ppp, unsigned long arg)
{
	...
	if (data.transmit) {
		...
		if (state != 0) {
			struct timer_list *timer = &(((struct ppp_mppe_state *)state)->ppp_mppe_block_timer);
			
			/*if(ppp->xc_state)
			{
				printk(KERN_EMERG "del_timer ppp[%p] state[%p] timer[%p] function[%p]-------------------------\n", ppp, ppp->xc_state, &(((struct ppp_mppe_state *)(ppp->xc_state))->ppp_mppe_block_timer), (((struct ppp_mppe_state *)(ppp->xc_state))->ppp_mppe_block_timer).function);
				del_timer_sync(&(((struct ppp_mppe_state *)(ppp->xc_state))->ppp_mppe_block_timer));
			}*/
			ppp_xmit_lock(ppp);
			...
			init_timer(timer);
			timer->expires = jiffies + 0x20;
			timer->data = (unsigned long)ppp;
			timer->function = ppp_xmit_process;
			//add_timer(timer);
			//printk(KERN_EMERG "init_timer ppp[%p] state[%p] timer[%p] function[%p]-------------------------\n", ppp, state, &(((struct ppp_mppe_state *)state)->ppp_mppe_block_timer), (((struct ppp_mppe_state *)state)->ppp_mppe_block_timer).function);
