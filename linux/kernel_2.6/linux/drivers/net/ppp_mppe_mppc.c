 ...
 *
 * The code is based on MPPE kernel module written by �p� Magos�yi and
...
#if 0
// this defination is moved to include/linux/ppp-comp.h, to be used in ppp_generic.c
/*
...
//Edit by Robert 2008/05/30
static void
get_new_key_from_sha(struct ppp_mppe_state *state)
{
...
//-----Edit by Robert 2008/05/30--------------
//-----Fix kernel crash bug, it's stack problem-----
static void
mppe_change_key(struct ppp_mppe_state *state, int initialize)
{
   ...
   //Edit by Robert 2008/05/30
    ...
    get_new_key_from_sha(state);
	 ...
	 //Edit by Robert 2008/05/30 
	 // if (!strlen(interim_key)) return;
   //state->keylen = sizeof(interim_key);
    ...
    if (initialize) {  	
		...
		arc4_setkey(state, state->sha1_digest, state->keylen);
		arc4_encrypt(state, state->sha1_digest, state->keylen, state->session_key);
    }
...
mppe_increase_ccount(struct ppp_mppe_state *state)
{
	...
	if (state->stateless) {
		...
		//if ((state->ccount & 0xff) == 0xff && !state->xmit_fail) {
			mppe_change_key(state, 0);
	    }
	}
    }
...
mppe_alloc(unsigned char *options, int opt_len, int comp)
{
	...
	/* Load ARC4 algorithm */
	    ...
	    //state->arc4_tfm_o = crypto_alloc_tfm("arc4", 0);
	      ...
	      int isize, int osize)
{
	...
	if (!state->mppc && state->mppe) { /* Do only encryption */
	    ...
	    //memcpy(state->arc4_tfm_o, state->arc4_tfm, sizeof(struct crypto_tfm)-sizeof(struct crypto_alg *));
	    //memcpy(state->arc4_tfm_o->__crt_alg, state->arc4_tfm->__crt_alg, sizeof(struct crypto_alg));

	    arc4_encrypt(state, ibuf, isize - off, wptr);
	    olen = isize - (off - 2) + MPPE_OVHD;
	    (state->stats).inc_bytes += olen;
	    (state->stats).inc_packets++;
	    //printk(KERN_EMERG "isize[%d] olen[%d] ==========================\n", isize, olen);
