...
#ifdef CONFIG_NK_KMALLOC_DEBUG

struct ttp *ttp_head=0;
int ttp_deb_start=0;

EXPORT_SYMBOL(ttp_deb_start);
EXPORT_SYMBOL(ttp_head);

void ttp_entry_ins(void *pointer,const char *func,int line)
{
	struct ttp *new;

	new=kmalloc_a( sizeof(struct ttp), GFP_ATOMIC);
	if(new) {
		new->next = ttp_head;
		new->alloc = pointer;
		new->line = line;
		new->flag = 0;
		if(strlen(func)<200)		
			strcpy(new->function,func);
		else {
			int count=0;
			for( ; count < 199;count++)
				new->function[count]=func[count];
			new->function[199]='\0';
		}
		ttp_head = new;
	}
...
void ttp_entry_del(void *pointer)
{
	struct ttp **prv;
	struct ttp *temp=0;

	for( prv=&ttp_head; *prv != 0; prv = &((*prv)->next) ) {
		if( (*prv)->alloc == pointer) {
			temp = *prv;
			*prv = temp->next;
			break;
		}
...
void *kmalloc_temp(size_t size, gfp_t flags,const char *function,int line)
{
	void *temp;

	temp = kmalloc_a(size,flags);
	if( (size <= CONFIG_MEM_UPBOUND) && (size >= CONFIG_MEM_LOWBOUND) && temp && ttp_deb_start){
		unsigned long fls;
		local_irq_save(fls);
		ttp_entry_ins(temp,function,line);
		local_irq_restore(fls);
	}
	return temp;
}
EXPORT_SYMBOL(kmalloc_temp);
#endif
