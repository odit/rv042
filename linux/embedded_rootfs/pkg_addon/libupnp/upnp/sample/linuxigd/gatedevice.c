...
int AddPortMapping(struct Upnp_Action_Request *ca_event)
{
	...
	syslog(LOG_DEBUG, "[AddPortMapping]eport:%s, proto:%s, iport:%s, ip:%s, en:%s, desc:%s",
		           ext_port, proto, int_port, int_ip, bool_enabled, desc);
...
int GetGenericPortMappingEntry(struct Upnp_Action_Request *ca_event)
{
	...
	char cmdBuf[40], ip[255], enabled[10], id[10];
	char forwarding[255], eport[20], iport[20], prot[20];
	char number[20], enable[10], name[100];


	//mapindex = GetFirstDocumentItem(ca_event->ActionRequest, "NewPortMappingIndex");
	//syslog(LOG_DEBUG, "mapindex=%d", atoi(mapindex));
	//if(g_debug) syslog(LOG_DEBUG, "g_index=%d", g_index);
	sprintf(cmdBuf, "UPNP_FORWARDING NUMBER");
	kd_doCommand(cmdBuf, CMD_PRINT, ASH_DO_NOTHING, number);

	if(g_index<atoi(number))
	{
		sprintf(cmdBuf, "UPNP_FORWARDING ID %d", g_index+1);
		...
		//{
			...
			//sprintf(result_param, "<NewRemoteHost>%s</NewRemoteHost><NewExternalPort>%s</NewExternalPort><NewProtocol>%s</NewProtocol><NewInternalPort>%s</NewInternalPort><NewInternalClient>%s</NewInternalClient><NewEnabled>%d</NewEnabled><NewPortMappingDescription>mymelody</NewPortMappingDescription><NewLeaseDuration>%li</NewLeaseDuration>", temp->m_RemoteHost, temp->m_ExternalPort, temp->m_PortMappingProtocol, temp->m_InternalPort, temp->m_InternalClient, temp->m_PortMappingEnabled, temp->m_PortMappingLeaseDuration);
			...
			if(!strcmp(prot, "6")) strcpy(prot, "TCP");
			else if(!strcmp(prot, "17")) strcpy(prot, "UDP");

// 			sprintf(result_param, "<NewRemoteHost>%s</NewRemoteHost><NewExternalPort>%s</NewExternalPort><NewProtocol>%s</NewProtocol><NewInternalPort>%s</NewInternalPort><NewInternalClient>%s</NewInternalClient><NewEnabled>%d</NewEnabled><NewPortMappingDescription>%s</NewPortMappingDescription><NewLeaseDuration>%li</NewLeaseDuration>",
// 			temp->m_RemoteHost, eport,
// 			prot, iport,ip, atoi(enabled),
// 			name,temp->m_PortMappingLeaseDuration);
			sprintf(result_param, "<NewRemoteHost></NewRemoteHost><NewExternalPort>%s</NewExternalPort><NewProtocol>%s</NewProtocol><NewInternalPort>%s</NewInternalPort><NewInternalClient>%s</NewInternalClient><NewEnabled>%d</NewEnabled><NewPortMappingDescription>%s</NewPortMappingDescription><NewLeaseDuration>0</NewLeaseDuration>",
			eport,
			prot, iport,ip, atoi(enabled),
			name);
			...
			action_succeeded = 1;
			g_index++;
			//syslog(LOG_DEBUG, "result_param=%s", result_param);
		//}
		//else
		//	syslog(LOG_DEBUG, "temp=NULL");
		...
		if (action_succeeded)
		{
			ca_event->ErrCode = UPNP_E_SUCCESS;
				...
				ca_event->ActionResult = ixmlParseBuffer(resultStr);
				//syslog(LOG_DEBUG, "resultStr=%s", resultStr);
		}
		else
		{
...
int GetSpecificPortMappingEntry(struct Upnp_Action_Request *ca_event)
{
   ...
   //syslog(LOG_DEBUG, "[yami]GetSpecificPortMappingEntry, g_index=%d", g_index);

//   if ((ext_port = GetFirstDocumentItem(ca_event->ActionRequest, "NewExternalPort"))
//      && (proto = GetFirstDocumentItem(ca_event->ActionRequest,"NewProtocol")))
//   {
      //syslog(LOG_DEBUG, "proto=%s, ext_port=%s", proto, ext_port);
      //if ((strcmp(proto, "TCP") == 0) || (strcmp(proto, "UDP") == 0))
     // {
		sprintf(cmdBuf, "UPNP_FORWARDING ID %d", g_index);
		kd_doCommand(cmdBuf, CMD_PRINT, ASH_DO_NOTHING, forwarding);
		//syslog(LOG_DEBUG, "UPNP_FORWARDING ID %d=%s", g_index, forwarding);
		name_get_value(forwarding, "START", eport, sizeof(eport), NULL);
		name_get_value(forwarding, "END", iport, sizeof(iport), NULL);
		name_get_value(forwarding, "IP", ip, sizeof(ip), NULL);
		name_get_value(forwarding, "ENABLED", enabled, sizeof(enabled), NULL);
		...
		sprintf(result_param,"<NewInternalPort>%s</NewInternalPort><NewInternalClient>%s</NewInternalClient><NewEnabled>%d</NewEnabled><NewPortMappingDescription>%s%s</NewPortMappingDescription><NewLeaseDuration>0</NewLeaseDuration>",
			iport,
			ip,
			atoi(enabled),
			name,
			( ( atoi ( id ) > 20 ) ? "" : " [MICROSOFT]" ) );
		//syslog(LOG_DEBUG, "result_param=%s", result_param);
    		action_succeeded = 1;
...
int DeletePortMapping(struct Upnp_Action_Request *ca_event)
{
      ...
      (proto = GetFirstDocumentItem(ca_event->ActionRequest, "NewProtocol"))))
   {
     ...
     //if(g_debug) syslog(LOG_DEBUG, "DeletePortMapping: eport=%s", ext_port);
     ...
     if ((strcmp(proto, "TCP") == 0) || (strcmp(proto, "UDP") == 0))
     {
	    ...
	    //if(g_debug) syslog(LOG_DEBUG, "[yami]result = %d", result);
         ...
         {
            //if(g_debug) syslog(LOG_DEBUG, " Do pmlist_DeletePortMapping");
	    pmlist_DeletePortMapping(1, proto, ext_port, NULL, NULL);
