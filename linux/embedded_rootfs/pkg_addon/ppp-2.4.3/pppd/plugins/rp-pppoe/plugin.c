...
"$Id: plugin.c 2375 2009-08-17 02:15:48Z richie $";
...
#ifdef CONFIG_NK_PADI_RETRY
    /*2006/2/14 trenchen : support linksys PADI retry interval*/
    int count=0;
    char PathName[20];
    char InterFace[10];
#endif
...
#ifdef CONFIG_NK_PADI_RETRY
	    /*2006/2/14 trenchen : support linksys PADI retry interval*/
	    for(count=0; count < CONFIG_NK_NUM_WAN; count++) {
	    	sprintf(PathName,"WAN%d WANINTERFACE",count+1);
		kd_doCommand( PathName, CMD_PRINT, ASH_DO_NOTHING, InterFace);

		if( !strcmp(devnam,InterFace) ) {
			WanNump = count+1;
			break;
		}
	    }
