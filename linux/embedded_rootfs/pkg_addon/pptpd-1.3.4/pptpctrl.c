 ...
 *
 * $Id: pptpctrl.c 4799 2010-04-28 01:46:24Z trenchen $
 */
...
int main(int argc, char **argv)
{
	...
	if (pppfork > 0) //Edit by Robert 2008/04/18 Reference 1.3.2
...
{
	char *pppd_argv[40];
    char lan_ip[20], wins_ip[20], dns1_ip[20], dns2_ip[20];
	int an = 0;
	sigset_t sigs;
...
#else
	//Add by Robert 2008/04/18 Reference 1.3.2
	pppd_argv[an++] = "pptpd";
...
#if 1
    {   /* add by chihmou 20090119, support Winserver and DNS */
        int status = 0x0;

        if((kd_doCommand("DHCP WINSERVER", 3, 0, wins_ip) == 0)
          && (strcmp(wins_ip, "0.0.0.0"))) 
            status |= 0x1;  // Get Winserver IP

        if((kd_doCommand("SYSTEM LAN", 3, 0, lan_ip) == 0)
          && (strcmp(lan_ip, "0.0.0.0"))) 
            status |= 0x2;  // Get LAN IP

        if((kd_doCommand("DHCP DNS1", 3, 0, dns1_ip) == 0)
          && (strcmp(dns1_ip, "0.0.0.0"))) 
            status |= 0x4;  // Get DNS IP

        if((kd_doCommand("DHCP DNS2", 3, 0, dns2_ip) == 0)
          && (strcmp(dns2_ip, "0.0.0.0"))) 
            status |= 0x8;  //Get DNS IP

        pppd_argv[an++] = "ms-wins";
        if(status & 0x1)
            pppd_argv[an++] = wins_ip;
        else if(status & 0x2)
            pppd_argv[an++] = lan_ip;
        else
            pppd_argv[an++] = "192.168.1.1";
        pppd_argv[an++] = "ms-wins";
        pppd_argv[an++] = "0.0.0.0";

        pppd_argv[an++] = "ms-dns";
        if(status & 0xC)
        {
            pppd_argv[an++] = dns1_ip;
            pppd_argv[an++] = "ms-dns";
            pppd_argv[an++] = dns2_ip;
        }
        else
        {
            if(status & 0x2)
...
#ifdef CONFIG_NK_PPTPSERVER_MPPE
	/*20100428 trenchen : support pptp server using mppe*/	
	pppd_argv[an++] = "mppe";
	pppd_argv[an++] = "required";
#endif

#ifdef CONFIG_MODEL_RV0XX
	pppd_argv[an++] = "lcp-echo-interval";
	pppd_argv[an++] = "6";
	pppd_argv[an++] = "lcp-echo-failure";
	pppd_argv[an++] = "5";
#else
	pppd_argv[an++] = "lcp-echo-interval";
	pppd_argv[an++] = "30";
