 ...
 *
 * $Id: pptpd.c 2129 2009-02-27 04:25:35Z david $
 */
...
static void reReadConfile(int sig)
{
	...
	free(configFile);
	syslog(LOG_WARNING, "Received SIGUSR1:reReadConfile");
}
#endif

static void reset_slot(int sig)
{
    FILE *fp_status;

    if((fp_status = fopen("/tmp/.pptp.disconnect", "r")) != NULL)
    {
        char buffer[MAX_CONFIG_STRING_SIZE];
        char pid[16];
        int num;

        while((fgets(buffer, MAX_CONFIG_STRING_SIZE - 1, fp_status)) != NULL)
        {
            sscanf(buffer, "%s", pid);
            if((num = slot_find_by_pid(atoi(pid))) != -1)
            {
                slot_set_pid(num, 0);
            }
        }
        fclose(fp_status);
        system("rm /tmp/.pptp.disconnect");
    }
}
...
#ifdef CONFIG_SUPPORT_PPTPD_READ_CONfIGFILE_BY_SIGNAL
	signal(SIGUSR1, reReadConfile);
#endif
    signal(SIGUSR2, reset_slot);
...
typedef struct used_ip_list{
    char ip[16];
    char pid[16];
    struct used_ip_list *next;
}used_ip_list
;

struct used_ip_list *add_used_ip_to_list(
            struct used_ip_list *pused_ip_list, char *pid, char *ip_remote)
{
    struct used_ip_list *tmp;

    tmp = (used_ip_list*)malloc(sizeof(used_ip_list));
    if(tmp)
    {
        bzero(tmp, sizeof(used_ip_list));
        sprintf(tmp->ip, ip_remote);
        sprintf(tmp->pid, pid);
        tmp->next = pused_ip_list;
        pused_ip_list = tmp;
    }
    return pused_ip_list;
}

void free_used_ip_list(struct used_ip_list *pused_ip_list)
{
    if(pused_ip_list)
    {
        if(pused_ip_list->next)
            free_used_ip_list(pused_ip_list->next);
...
int is_exist_in_used_ip_list(struct used_ip_list *pused_ip_list, char *ip)
{
    int ret = 0;
    struct used_ip_list *tmp = pused_ip_list;
...
static void processIPStr(int type, char *ipstr)
{
    ...
    FILE *fp_status;
    struct used_ip_list *pused_ip_list = NULL;

	num = 0;

    /* check /tmp/.pptp.status for connected */
    if((type == REMOTE) &&
        ((fp_status = fopen("/tmp/.pptp.status", "r")) != NULL))
    {
        char buffer[MAX_CONFIG_STRING_SIZE];
        char username[128], passwd[128], ip_local[16], ip_remote[16], pid[16];

        while((fgets(buffer, MAX_CONFIG_STRING_SIZE - 1, fp_status)) != NULL)
        {
            sscanf(buffer, "%s %s %s %s %s "
                    , username, passwd, ip_local, ip_remote, pid);
            pused_ip_list = add_used_ip_to_list(pused_ip_list, pid, ip_remote);
        }
        fclose(fp_status);
    }
	...
	while (!bail) {
		...
		if (!isIpRange(tmpstr2)) {
			...
			if (num == pptp_connections) {
                ...
                free_used_ip_list(pused_ip_list);
				return;
			}
#if DEBUG_IP_PARSER
            ...
            {
                int pid = 0;
                if((pid = is_exist_in_used_ip_list(pused_ip_list, tmpstr7)) > 0)
                    slot_set_pid(num, pid);
