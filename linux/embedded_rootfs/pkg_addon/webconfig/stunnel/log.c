...
/* --> 2005-12-07 Jerry Wu modified, A temporary solution for solving the problem
   that some unexpected words padded behind normal message shown on System Log
   UI page.
   Temporary solution:
   1. Every message have to be terminated by '\n'. (see pppoe/utils.c - poe_logit())
   2. Change '\n' to '\0' before printing to System Log. (see syslog/syslog.c - unix_socket_handler())
*/
//        syslog(level, "%s", text);
        syslog(level, "%s\n", text);
/*<-- */
