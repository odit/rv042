...
static int init_remote(CLI *c)
{
    ...
    if (fd>=max_fds)
    {
        log(LOG_ERR, "Remote file descriptor out of range (%d>=%d)", fd, max_fds);
        closesocket(fd);
        return -1;
    }
#endif
...
static int transfer(CLI *c) { /* transfer data */
    ...
    while(((sock_rd||c->sock_ptr)&&ssl_wr)||((ssl_rd||c->ssl_ptr)&&sock_wr)) {
        ...
        if(sock_wr && FD_ISSET(c->sock_wfd->fd, &wr_set)) {
			/* send stunnel header massage to httpd before real data --> Kide 2004/06/25 */
			/* add 60443 port support for QuickVPN Second trial --> Encounter 2005/12/27 */
			if (c->sock_bytes == 0 && (!strcmp(c->opt->servname, "https") ||
			                           !strcmp(c->opt->servname,  "QVPN")  ) )
			{
				char	stunnel_hdr[100];
				int		addr_sz = sizeof(struct sockaddr_in);
				struct sockaddr_in	l_ip;

				getsockname(c->local_rfd.fd, (struct sockaddr *)&l_ip, &addr_sz);
				sprintf(stunnel_hdr, "stunnel remote/local: %s/%s\n",
									c->accepting_address, inet_ntoa(l_ip.sin_addr));
				// send stunnel header
				writesocket(c->sock_wfd->fd, stunnel_hdr, strlen(stunnel_hdr));
			}
			// <-- Kide 2004/06/25
