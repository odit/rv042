...
int name2nums(char *, char *, unsigned long **, u_short *);
...
typedef struct {
    ...
    unsigned long *resolved_addresses; /* List of IP addresses for delayed lookup */
...
#endif
int write_blocking(CLI *, int fd, unsigned char *, int);
int read_blocking(CLI *, int fd, unsigned char *, int);
