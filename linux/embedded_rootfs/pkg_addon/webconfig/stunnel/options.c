...
static int host2nums(char *, unsigned long **);
...
static char *service_options(CMD cmd, LOCAL_OPTIONS *section, char *opt, char *arg)
{
    if (cmd==CMD_DEFAULT || cmd==CMD_HELP)
    {
        log_raw(" ");
    ...
    switch (cmd)
    {
        ...
        case CMD_DEFAULT:
            break;
        case CMD_HELP:
            log_raw("%-15s = [host:]port connect remote host:port", "connect");
            break;
    }
...
int name2nums(char *name, char *default_host, unsigned long **names, u_short *port) {
...
static int host2nums(char *hostname, unsigned long **hostlist) {
    ...
    if(ip+1) { /* (signed)ip!=-1 */
        *hostlist=calloc(2, sizeof(unsigned long));
        if (!*hostlist) {
    ...
    for(results=0, tab=h->h_addr_list; *tab; tab++)
        results++;
    *hostlist=calloc(results+1, sizeof(unsigned long)); /* allocate memory */
    if (!*hostlist) {
        log(LOG_ERR, "Memory allocation error");
        return 0;
    }
    for(i=0; i<results; i++) /* copy addresses */
        (*hostlist)[i]=*(unsigned long *)(h->h_addr_list[i]);
    (*hostlist)[results]=-1;
