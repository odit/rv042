...
static int cifs_client(CLI *c) {
    ...
    unsigned char request_dummy[4] = {0x81, 0, 0, 0}; /* a zero-length request */
...
static int cifs_server(CLI *c) {
    ...
    unsigned char response_access_denied[5] = {0x83, 0, 0, 1, 0x81};
    unsigned char response_use_ssl[5] = {0x83, 0, 0, 1, 0x8e};
    unsigned short len;

    if(read_blocking(c, c->local_rfd.fd, buffer, 4)<0) /* NetBIOS header */
        return -1;
    len=buffer[3];
    len|=(unsigned short)(buffer[2]) << 8;
