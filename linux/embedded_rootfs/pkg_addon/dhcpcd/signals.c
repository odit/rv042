...
void writePidFile(pid_t pid)
{
  ...
  if ( fp == NULL )
    {
      syslog(LOG_INFO,"writePidFile: fopen: %m\n");
      exit(1);
    }
  fprintf(fp,"%u\n",pid);
  fclose (fp);
      ...
      if ( currState == &dhcpBound )
	      ...
	      else
		{
		  if ( currState == &dhcpReboot )
		    ...
		    {
		        syslog(LOG_INFO,"timed out waiting for a valid DHCP server response\n");
	   		return;		// Ryoko 2005/07/07
		    }
		}
	    }
        }
