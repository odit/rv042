...
#ifdef NK_CONFIG_IPV6
void NK_set_get_ipv6_now(int type)
{
	get_ipv6_now=type;
}
#endif
	...
	if (evaluate_option_cache (&ds, packet, lease, client_state,
		...
		fprintf(leaseFile, "%soption %s%s%s %s;\n", preamble,
			...
			pretty_print_option(oc->option, ds.data, ds.len,
					    1, 1));
#ifdef NK_CONFIG_IPV6
	dhclient_write_GW_DNS_to_data_ipv6(get_ipv6_now, &NK_interface, oc->option->name,pretty_print_option(oc->option, ds.data, ds.len, 1, 1));
#endif
		data_string_forget (&ds, MDL);
	}
}
...
#ifdef NK_CONFIG_IPV6
	memset (&NK_interface, 0, sizeof(NK_interface));
	sprintf(NK_interface,"%s",client->interface->name);
#endif
		...
		for (addr = ia->addrs ; addr != NULL ; addr = addr->next) {
...
#ifdef NK_CONFIG_IPV6
			{
				...
				stat = fprintf(leaseFile,
					       "    iaaddr %s {\n",
					       piaddr(addr->address));
#ifdef NK_CONFIG_IPV6 /* write the ipv6 ip to ISP's WAN_V6 in the database*/
				dhclient_write_GW_DNS_to_data_ipv6(get_ipv6_now, &NK_interface, "iaaddr",piaddr(addr->address));
			}
#endif
			else
				stat = fprintf(leaseFile,
