...
start_bound(struct client_state *client)
{
	...
	NK_set_get_ipv6_now(1); /* When device get ip, it sets flag and writing gateway and dns to database */
#endif

	write_client6_lease(client, lease, 0, 1);
	
#ifdef NK_CONFIG_IPV6
	NK_set_get_ipv6_now(0);/* After writes gateway and dns to database, init the flag.*/
#endif
