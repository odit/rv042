...
void nk_get_ipv6_hostname_of_client(unsigned char *packet_data, int data_length)
{
	int i=0,length_location=6,option_length=0,option_location=4,option_type=0;

	while(option_location < data_length && length_location < data_length)
	{
		option_type=packet_data[option_location]<< 8 | packet_data[option_location+1];
		option_length=packet_data[length_location]<< 8 | packet_data[length_location+1];
		
		if(option_length-2 > NK_HOSTNAME_MAXIMUM_LENGTH) /* hostname maximum length*/
			option_length=NK_HOSTNAME_MAXIMUM_LENGTH+2;
			
		if(option_type==39) /* Fully Qualified Domain Name*/
		{
			memset(&NK_hostname[0],0,sizeof(NK_hostname));
			strncpy(&NK_hostname[0],&packet_data[length_location+4],option_length-2);
			for(i=0;i<option_length-2;i++)
			{
				if(NK_hostname[i] >0 && NK_hostname[i] <32) /* It's avoid the question that the char can't print.*/
					NK_hostname[i]=46; /* It is '.' in the ascii table*/
			}
		}
		option_location+=option_length+4;
		length_location+=option_length+4;
	}
	if(!strlen(NK_hostname))
	{
		strncpy(&NK_hostname[0],"unknown",strlen("unknown"));
	}
}
#endif
...
#ifdef NK_CONFIG_IPV6
	nk_get_ipv6_hostname_of_client((unsigned char *)packet->raw, packet->packet_length);
#endif
	/*
	 * Validate our input.
	 */
