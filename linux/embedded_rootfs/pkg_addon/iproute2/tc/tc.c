...
int main(int argc, char **argv)
{
	...
	int do_batching = 0;
	char *batchfile = NULL;
//=========add to support qos file
	FILE *qos_fd, *qos_fd_1;
	char line[400];
	char *largv[100];
	int largc=0;
	char buf[400], test[400], remove_file[100], test1[16];
	int i;
//=============================
...
//===========================================================================
	if (argc > 1 && matches(argv[1], "netklass") == 0) {
		if ((qos_fd = fopen("/tmp/qos_file", "r")) != NULL)
		{
			fscanf(qos_fd,"%s\n",test);
			...
			for(i=0;!feof(qos_fd);i++)
			{
				fscanf(qos_fd,"%s\n",test);
				if (strcmp(test, "eoc") != 0)//not end of line
				{
					printf("===%s===\n", test);
					strcat(buf,test);
					strcat(buf," ");
				...
				else
				{
					fprintf(stderr, "==%s==\n", buf);
					largc = 0;
					largv[largc]=strtok(buf, " ");
					while ((largv[++largc]=strtok(NULL, " ")) != NULL){};

					if (matches(largv[1], "qdisc") == 0)
						do_qdisc(largc-2, largv+2);
					if (matches(largv[1], "class") == 0)
						do_class(largc-2, largv+2);
					if (matches(largv[1], "filter") == 0)
						do_filter(largc-2, largv+2);
...
//===============================================================
		...
		sprintf(test1,"eoc\n");
		fwrite(test1, sizeof(char), strlen(test1), qos_fd_1);
		fclose(qos_fd_1);
//===================================================================
		if ((qos_fd = fopen("/tmp/smart_qos_file", "r")) != NULL)
		{
			fscanf(qos_fd,"%s\n",test);
			...
			fclose(qos_fd);
			fprintf(stderr, "======================\n");
			fprintf(stderr, "Set smart qos finish!!\n");
			fprintf(stderr, "======================\n");
			//return 0;
		}
		else
