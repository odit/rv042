...
int print_route(const struct sockaddr_nl *who, struct nlmsghdr *n, void *arg)
{
	...
	if (tb[RTA_PROTOCOL_TYPE]){
		fprintf(fp, "protocol_type %d ",*(__u32*)RTA_DATA(tb[RTA_PROTOCOL_TYPE]));
	}
	if (tb[RTA_START_DST_PORT]){
		fprintf(fp, "dst_port %d ",ntohs(*(__u32*)RTA_DATA(tb[RTA_START_DST_PORT])));
	}
	if (tb[RTA_START_SRC_PORT]){
		fprintf(fp, "src_port %d ",ntohs(*(__u32*)RTA_DATA(tb[RTA_START_SRC_PORT])));
	}
	/********************************************************************************/
