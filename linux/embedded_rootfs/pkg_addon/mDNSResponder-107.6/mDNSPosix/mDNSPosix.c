...
void GetUserSpecifiedRFC1034ComputerName(domainlabel *const namelabel)
	{
	int len = 0;
...
void GetUserSpecifiedFriendlyComputerName(domainlabel *const namelabel)
	{
...
mDNSexport int ParseDNSServers(mDNS *m, const char *filePath)
	...
	fclose(fp);  // 20100126 chihmou add
