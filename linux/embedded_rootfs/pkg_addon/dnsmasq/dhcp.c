...
extern SEM_ID dhcpsMutexSem;			   /* Synchronization for lease entry adds. */
#endif
...
void load_dhcp(char *file, char *suffix, time_t now, char *hostname)
{
  ...
  fclose(fp);
	...
	if (semTake(dhcpsMutexSem, WAIT_FOREVER) == OK)
	{
		struct hash_member *bind = bindlist;
		while (bind)
		{
			struct dhcp_binding *dhcp = (struct dhcp_binding *)bind->data;
			char *dot;
			strncpy(hostname, dhcp->res->hostname, MAXDNAME);
			...
			if (*hostname)
			{
				...
				{ 
					...
					{ /* suffix and lease has ending: must match */
						if (strcmp(dot+1, suffix) != 0)
							syslog(LOG_WARNING, "Ignoring DHCP lease for %s because it has an illegal domain part", hostname);
						else
					  		process_lease(&spares, hostname, dhcp->res->ip_addr, dhcp->expire_epoch, F_REVERSE);
					}
					else
					{ /* suffix exists but lease has no ending - add lease and lease.suffix */
						process_lease(&spares, hostname, dhcp->res->ip_addr, dhcp->expire_epoch, 0);
						strncat(hostname, ".", MAXDNAME);
						strncat(hostname, suffix, MAXDNAME);
						hostname[MAXDNAME-1] = 0; /* in case strncat hit limit */
						/* Make FQDN canonical for reverse lookups */
				...
				{ /* no suffix */
					if (dot) /* no lease ending allowed */
				      		syslog(LOG_WARNING, "Ignoring DHCP lease for %s because it has a domain part", hostname);
				    	else
				    		process_lease(&spares, hostname, dhcp->res->ip_addr, dhcp->expire_epoch, F_REVERSE);
				}
			}
