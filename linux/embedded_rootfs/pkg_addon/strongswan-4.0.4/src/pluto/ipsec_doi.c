 ...
 * for more details.
 *
 * RCSID $Id: ipsec_doi.c 7042 2010-11-06 04:53:07Z david.kao $
 */
    ...
    u_int16_t type)
{
    ...
    else if (IS_ISAKMP_ENCRYPTED(state) && IS_ISAKMP_ENCRYPTED(st->st_state)) {
//Charles: If the peer's status far is beyond us and sending the key payload inside the informational packets, we have to check both side to see if we really need to excrypt the packet
...
accept_delete(struct state *st, struct msg_digest *md, struct payload_digest *p)
{
    ...
    size_t sizespi;
    int i,qkn_ignore=0;
//Max - Keep from retransmission at the same time
    int rand_delay;
    struct state *tmp_st;
//<<
...
//Max - Keep from retransmission at the same time
			//event_schedule(EVENT_SA_REPLACE, DELETE_SA_DELAY, dst);
                        rand_delay = rand() % DELETE_SA_DELAY;
			event_schedule(EVENT_SA_REPLACE, rand_delay, dst);
//<<
		    }
		}
		else
		{
			...
			(check_phase2_SA(st) || st->st_connection ->cannot_kill_last_SA == 1))
			{
				st->st_connection->killed_ipsec_sa = st->st_connection->newest_ipsec_sa;

			}
			else if(!strncmp(st->st_connection->name,"qknips",6) && (st->st_connection->newest_ipsec_sa == dst->st_serialno))
			{
    ...
    , lset_t policy, unsigned long try)
{
    ...
    int vids_to_send = 0;
//Max - keep from as initiator
    if(strstr(c->name,"qkn") )
...
/* Encounter: we should control this by tunnel rather than whole strongswan */
	...
	/* purpose 0012506  author : lucy.jiang	updater: Frank 	date £º2010-06-25 */
...
, so_serial_t replacing)
{
	...
	*r_hash_start;	/* start of what is to be hashed */
    ...
    st->st_connection = c;
#if 0    
   /* Add by Daniel Cheng 2008.11.18 phase2 FSM -->
     */
    /* Daniel Cheng <-- */

/* Max modify 20090210  FSM not work for Quick VPN */
    if(strstr(st->st_connection->name,"g2g")&& check_phase2_FSM(st,PHASE2_FSM_INITIATOR))
    {
	plog("max max [ipsec_doi.c][quick_outI1] FSM check goto drop ...");//Daniel Add
...
//both ipsec_doi.c and nk_ipsec.h set IPSEC_IPC_DEL_CONN_BYNAME
#define IPSEC_IPC_DEL_CONN_BYNAME 2
//both ipsec_doi.c and nk_ipsec.c set DNS_NAME_LEN
...
//<<
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Main Mode 1st packet");

    RETURN_STF_FAILURE(preparse_isakmp_sa_body(&sa_pd->payload.sa
	, &sa_pd->pbs, &ipsecdoisit, &proposal_pbs, &proposal));

	if(memcmp(md->hdr.isa_rcookie, zero_cookie, COOKIE_SIZE) == 0)	
	{
		st_icookie = find_state_by_icookie(md->hdr.isa_icookie);
		if(st_icookie)
		{
			if(st_icookie -> st_state == STATE_MAIN_R1 )
			{
				delete_event(st_icookie);
				st->st_retransmit = 0;
				event_schedule(EVENT_RETRANSMIT,0,st_icookie);	
    				loglog(RC_LOG_SERIOUS,"[Max][Max][Max] isa_rcookie");
			}
			else
    				loglog(RC_LOG_SERIOUS,"[Max][Max][Max] STF_IGNORE");
	    	return STF_IGNORE;
		}
	}
...
//<
	st->st_connection ->cannot_kill_last_SA = 1;
    return STF_OK;
}
...
//<<
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Initiator Received Main Mode 2nd packet");
...
//max add for vpn backup
	memset(tmp,0,DNS_NAME_LEN);
        if(!strstr(st->st_connection->name,"vbk"))
	{
...
main_inI2_outR2(struct msg_digest *md)
{
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Main Mode 3rd packet");
...
main_inR2_outI3(struct msg_digest *md)
{
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Initiator Received Main Mode 4th packet");
...
, struct key_continuation *kc)
{
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Main Mode 5th packet");
...
{
     loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] >>> Initiator Receive Main Mode 6th packet");
...
//<<
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Aggressive Mode 1st packet");
...
aggr_inR1_outI2(struct msg_digest *md)
{
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Initiator Received Aggressive Mode 2nd packet");
...
aggr_inI2(struct msg_digest *md)
{
	...
	/* purpose : 0012506  author : lucy.jiang	updater: Frank.Chang date : 2010-06-25 */
	/* description : whether to check the hash                                */
	bool checkid = TRUE;

    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Aggressive Mode 3rd packet");
    ...
    /* HASH_I or SIG_I in */
    {
	/* purpose : 0012506  author : lucy.jiang	date  updater: Frank.chang: 2010-06-25                 */
...
quick_inI1_outR1(struct msg_digest *md)
{
   ...
   /* Add by Daniel Cheng 2008.11.18 phase2 FSM -->
    ...
    struct state *tmp_st;
    tmp_st = new_state();
    tmp_st->st_connection = c;
    if(strstr(c->name,"g2g") && check_phase2_FSM(tmp_st,PHASE2_FSM_RESPONDER))
    {
        pfree(tmp_st);
	return STF_INTERNAL_ERROR;
    }
    ...
    /* Daniel Cheng <-- */
#endif
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Quick Mode 1st packet");
...
*/
	...
	if((st->st_policy & POLICY_PFS) && st->st_pfs_group){
		if(!st->st_connection->alg_info_esp){
			loglog(RC_LOG_SERIOUS, "The DH Remote GROUP_DESCRIPTION is %d but the local DH GROUP_DESCRIPTION is not available",st->st_pfs_group->group);
			return STF_FAIL + NO_PROPOSAL_CHOSEN;
		}
		if(st->st_connection->alg_info_esp->esp_pfsgroup != st->st_pfs_group->group){
			loglog(RC_LOG_SERIOUS, "The Remote DH GROUP_DESCRIPTION %d do NOT match the local DH GROUP_DESCRIPTION %d",st->st_pfs_group->group,c->alg_info_esp->esp_pfsgroup);
			return STF_FAIL + NO_PROPOSAL_CHOSEN;
		}
	}

/* purpose	: 0012651 ,0012492
 * author	: lucy   updater: Frank.chang
 * date	: 2010-06-24 , 2010-06-30
 * description: fix vpn g to g protocol check
 */
	if(st->st_policy&POLICY_AUTHENTICATE){
	    loglog(RC_LOG_SERIOUS, "lucy debug POLICY_AUTHENTICATE");
		if(!st->st_ah.attrs.transid){
			loglog(RC_LOG_SERIOUS, "The local AUTHENTICATE Algorithm is %d ,but the remote  do not choose AUTHENTICATE ",st->st_connection->alg_info_auth);
			return STF_FAIL + NO_PROPOSAL_CHOSEN;
		}
		if(st->st_ah.attrs.transid!=st->st_connection->alg_info_auth){
			loglog(RC_LOG_SERIOUS, "The Remote AUTHENTICATE Algorithm %d ,do NOT match the local AUTHENTICATE Algorithm %d",st->st_ah.attrs.transid,st->st_connection->alg_info_auth);
			return STF_FAIL + NO_PROPOSAL_CHOSEN;
	    }
	}else{
		if(st->st_ah.attrs.transid){
			loglog(RC_LOG_SERIOUS, "The Remote AUTHENTICATE Algorithm %d ,but the local do not choose AUTHENTICATE",st->st_ah.attrs.transid);
			return STF_FAIL + NO_PROPOSAL_CHOSEN;
			}
	}
...
quick_inR1_outI2(struct msg_digest *md)
{
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Initiator Received Quick Mode 2nd packet");
    ...
    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] Quick Mode Phase 2 SA Established, IPSec Tunnel Connected ");
...
{
    struct state *const st = md->st;
    struct state *killed_st;

    /* HASH(3) in */
    CHECK_QUICK_HASH(md, quick_mode_hash3(hash_val, st)
	, "HASH(3)", "Quick I2");

    loglog(RC_LOG_SERIOUS,"[Tunnel Negotiation Info] <<< Responder Received Quick Mode 3rd packet");	
    ...
    killed_st = state_with_serialno(st->st_connection->killed_ipsec_sa);
    st->st_connection ->cannot_kill_last_SA = 0;

    if(killed_st)
    {
    	delete_event(killed_st);
    	event_schedule(EVENT_SO_DISCARD, 2, killed_st);
        st->st_connection->killed_ipsec_sa = 0;
    }
