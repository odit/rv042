 ...
 * for more details.
 *
 * RCSID $Id: kernel_alg.c 5848 2010-07-20 10:30:13Z dio.li $
 */
...
kernel_alg_esp_ok_final(u_int ealg, u_int key_len, u_int aalg, struct alg_info_esp *alg_info)
{
    ...
    /* 
      ...
      * author : 	Dio.Li	
      * date:		2010-07-19 
      * description : Cancel the "alg_info->alg_info_flags & ALG_INFO_F_STRICT" condition
      *                    because all need to check esp_info
      */
    if (ealg_insecure
	...
	if (alg_info)
	{
	    ...
	    {
		/* debug */
		loglog(RC_LOG_SERIOUS, "esp_ealg_id=%d-%d,esp_ealg_keylen=%d, key_len=%d,esp_aalg_id=%d-%d.\n",
		esp_info->esp_ealg_id, ealg,esp_info->esp_ealg_keylen, key_len, esp_info->esp_aalg_id,aalg);
