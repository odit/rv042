 ...
 * for more details.
 *
 * RCSID $Id: demux.c 5907 2010-07-22 13:49:15Z frank.chang $
 */
...
//#ifdef CONFIG_IPEC_AGGR_MODE
#if 1  
    case ISAKMP_XCHG_AGGR:  //2007/11/12 trenchen : support aggressive mode
#endif
...
//    case ISAKMP_XCHG_AGGR:
	...
	//s_time=cvmx_get_cycle();
    complete_state_transition(mdp, smc->processor(md));
	//e_time=cvmx_get_cycle();
	//loglog(RC_LOG_SERIOUS,"Total Cycle from state %s = %ld cycles", enum_name(&state_names, md->from_state),  e_time-s_time);
}
...
//Max test 0528
		    if(strstr(st->st_connection->name,"qkn") || strstr(st->st_connection->name,"grp"))
			    delay = VPN_CLIENT_RETRANSMIT_DELAY; 
		    else
...
//Max - initiator rekey timer shorter than responder 
/* purpose	:	0012939
 * author	:	Frank.Chang	
 * date	:	2010-07-22
