 ...
 * for more details.
 *
 * RCSID $Id: kernel.c 5150 2010-05-25 12:07:43Z david.kao $
 */
...
# define DEFAULT_UPDOWN	"_updown"
#endif

unsigned int get_netmask(unsigned int net1, unsigned int net2)
{
...
#else
    if(c->spd.that.client_id == ID_IPV4_ADDR_SUBNET){
	networkof(&sr->that.client, &ta);
	addrtot(&ta, 0, peerclientnet_str, sizeof(peerclientnet_str));//charles add
	maskof(&sr->that.client, &ta);
	addrtot(&ta, 0, peerclientmask_str, sizeof(peerclientmask_str));//charles add   
    }
    else if(c->spd.that.client_id == ID_IPV4_ADDR){
	networkof(&sr->that.client, &ta);
	addrtot(&ta, 0, peerclientnet_str, sizeof(peerclientnet_str));//charles add
	ta.u.v4.sin_addr.s_addr = 0xFFFFFFFF;
	addrtot(&ta, 0, peerclientmask_str, sizeof(peerclientmask_str));//charles add
    }
    else if(c->spd.that.client_id == ID_IPV4_ADDR_RANGE){
	ta.u.v4.sin_addr.s_addr = get_netmask(c->spd.that.client_addr1.u.v4.sin_addr.s_addr,c->spd.that.client_addr2.u.v4.sin_addr.s_addr);
	addrtot(&ta, 0, peerclientmask_str, sizeof(peerclientmask_str));//charles add 
        ta.u.v4.sin_addr.s_addr = c->spd.that.client_addr1.u.v4.sin_addr.s_addr & ta.u.v4.sin_addr.s_addr;
	addrtot(&ta, 0, peerclientnet_str, sizeof(peerclientnet_str));//charles add    
    }
    else{
	networkof(&sr->that.client, &ta);
...
, unsigned int op, const char *opname)
{
	...
	/* purpose: IPSec Stability
	* author: Frank
	* date: 2010-05-25
	* description: In some condition, pluto will crash.  We only add some error handler to avoid crash
	*/
	if(!sr)
	{
		loglog(RC_LOG_SERIOUS, "eroute_connection: sr is NULL !!!!!!!!!!!!!!!!!!!");
		return FALSE;
	}
	if(!opname)
	{
		loglog(RC_LOG_SERIOUS, "eroute_connection: opname is NULL !!!!!!!!!!!!!!!!!!!");
	    //return FALSE;
	}
    
	peer = &sr->that.host_addr;
    
	snprintf(buf2, sizeof(buf2), "eroute_connection %s", opname);

	if (proto == SA_INT)
	{
		if (!aftoinfo(addrtypeof(peer)))
		{
			plog("Frank:  pee ris NULL");
		}
		else
		{
