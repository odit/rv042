 ...
 * for more details.
 *
 * RCSID $Id: constants.h 5229 2010-06-02 11:50:29Z david.kao $
 */
...
#define VPN_CLIENT_RETRANSMIT_DELAY		2 /* 2 seconds */
...
#define ISAKMP_SA_ESTABLISHED_STATES  (LELEM(STATE_MAIN_R3) | LELEM(STATE_MAIN_I4) \
    | LELEM(STATE_MODE_CFG_R1) | LELEM(STATE_MODE_CFG_I2) | LELEM(STATE_AGGR_R2) | LELEM(STATE_AGGR_I2)  )

#define IS_PHASE1(s) ((STATE_MAIN_R0 <= (s) && (s) <= STATE_MAIN_I4) \
		   || (STATE_AGGR_R0 <= (s) && (s) <= STATE_AGGR_R2) \
		   || (STATE_MODE_CFG_R0 <= (s) && (s) <= STATE_MODE_CFG_I2))
#define IS_QUICK(s) (STATE_QUICK_R0 <= (s) && (s) <= STATE_QUICK_R2)
#define IS_ISAKMP_ENCRYPTED(s) ( (STATE_MAIN_I2 <= (s) && (s) <= STATE_MAIN_I4) || STATE_AGGR_I2 <= (s))
...
#if 1 //aggressive mode
#define POLICY_AGGRESSIVE LELEM(17)
#endif
#define POLICY_NATTRAVERSAL LELEM(18)
...
// Change the default value to 0 to let keep alive re-try forever.
