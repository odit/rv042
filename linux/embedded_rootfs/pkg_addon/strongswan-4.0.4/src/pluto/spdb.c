 ...
 * for more details.
 *
 * RCSID $Id: spdb.c 5048 2010-05-14 06:57:34Z charles.lin $
 */
...
// Encounter: We want null
#ifndef SUPPORT_ESP_NULL
#define SUPPORT_ESP_NULL
#endif
...
static struct db_attr espnone_attr[] = {
    { AUTH_ALGORITHM, AUTH_ALGORITHM_NONE },
    };
...
static struct db_trans espa_trans[] = {
    { ESP_DES, AD(espnone_attr) },
    { ESP_3DES, AD(espnone_attr) },
...
static struct db_trans ah_trans_md5[] = {
    { AH_MD5, AD(ah_HMAC_MD5_attr) }
    };
static struct db_trans ah_trans_sha1[] = {
    { AH_SHA, AD(ah_HMAC_SHA1_attr) }
    };
...
, u_int8_t np)
{
	...
	for (pn = 0; pn != pc->prop_cnt; pn++)
	{
	     ...
	     *  Replace AH proposals with runtime created one
	     */
		 if (!oakley_mode && p->protoid == PROTO_IPSEC_AH)
			{
				int auth_algid = st->st_connection->alg_info_auth;
				DBG(DBG_CONTROL | DBG_CRYPT,
						DBG_log("authenticate proposal: %d (2 md5,3 sha1)", auth_algid)
				)
				
				switch(auth_algid)
					{
					case AH_MD5:
						p->trans=&ah_trans_md5;
						p->trans_cnt=1;
						break;
					case AH_SHA:
						p->trans=&ah_trans_sha1;
						p->trans_cnt=1;
						break;
					default:
						p->trans=&ah_trans;
...
/* 2006/04/21 jane: init aggr mode sa according to ui setting */
{
    struct ike_info *ike_info = st->st_connection->alg_info_ike->ike;

    ta.encrypt = ike_info->ike_ealg; /* OAKLEY_ENCRYPTION_ALGORITHM */
    ta.encrypter = ike_alg_get_encrypter(ta.encrypt);
    ta.enckeylen = ta.encrypter->keydeflen;
    ta.hash = ike_info->ike_halg;  /* OAKLEY_HASH_ALGORITHM */
    ta.hasher = ike_alg_get_hasher(ta.hash);
...
//    oakley_tr_am[2].val = ta.auth; /* OAKLEY_AUTHENTICATION_METHOD */
    ...
    ta.group = lookup_group(ike_info->ike_modp); /* OAKLEY_GROUP_DESCRIPTION */

//log("ike_info: ike_ealg=[%d], ike_halg=[%d], ike_modp=[%d]", ike_info->ike_ealg, ike_info->ike_halg, ike_info->ike_modp);
//log("ta.encrypt=[%d], ta.hash=[%d], ta.group=[%d]", ta.encrypt, ta.hash, ta.group->group);
}
		   ...
		   , struct state *st)
{
	...
	if(!in_struct(&trans, &isakmp_isakmp_transform_desc, proposal_pbs, &trans_pbs)){
		loglog(RC_LOG_SERIOUS, "fail to parse ISAKMP Transform Payloads Oakley Proposal Payload");
		return BAD_PROPOSAL_SYNTAX;
	}
	attr_start = trans_pbs.cur;
	attr_len = pbs_left(&trans_pbs);
...
/* 
    Encounter: according to jane's patch, we choose what we want, not what we have 
*/
#if 0
		...
		if (val == st->st_connection->alg_info_ike->ike->ike_ealg)
		{
		    ta.encrypt = val;
		    ta.encrypter = ike_alg_get_encrypter(val);
		    ta.enckeylen = ta.encrypter->keydeflen;
		...
		{
		    ugh = builddiag("%s is not enabled for this connection"
			    , enum_show(&oakley_enc_names, val));
		}
		break;

	    case OAKLEY_HASH_ALGORITHM | ISAKMP_ATTR_AF_TV:
/* Encounter: choose what we want, not what we have ... */
#if 0
		...
		if (val == st->st_connection->alg_info_ike->ike->ike_halg)
		{
		    ta.hash = val;
		    ta.hasher = ike_alg_get_hasher(val);
		}
...
/* Encounter: choose want we want, not what we have */ 
		...
		if (val != st->st_connection->alg_info_ike->ike->ike_modp)
		{
		    ugh = builddiag("%s is not enabled for this connection"
...
/* Encounter: choose what we want, not what we have */
		if(val != st->st_connection->alg_info_ike->ike->ike_eklen)
		{
		    ugh = "peer proposed key length not enabled for "
			  "this connection";
		}
		ta.enckeylen = val;
		break;
...
/* Encounter: choose what we want, not what we have ...........*/
		...
		int cnt;
		ALG_INFO_ESP_FOREACH(c->alg_info_esp, esp_info, cnt){
			if ( (esp_attrs.transid == esp_info->esp_ealg_id)  // encrypt
			  && (esp_attrs.auth == esp_info->esp_aalg_id) 	// auth
			  && (esp_attrs.key_len == esp_info->esp_ealg_keylen)) {  // encrypt key len
				accept = TRUE;
				break;
			}
		}
