 ...
 * for more details.
 *
 * RCSID $Id: state.c 5232 2010-06-02 11:54:11Z david.kao $
 */
...
struct state *check_phase2_SA(struct state *from )
{
   struct state *st, *st_from = from;
    int i;

    if(st_from == NULL)
    	return 0;
    if(st_from->st_connection == NULL)
    	return 0;
	...
	for (st = statetable[i]; st != NULL; st = st->st_hashchain_next)
	{
		...
		if(st->st_state>=STATE_QUICK_R0 && st->st_state<=STATE_QUICK_R1 /*&& st!=st_from*/)
#if 1
		if(st->st_connection->spd.this.client_addr1.u.v4.sin_addr.s_addr == st_from->st_connection->spd.this.client_addr1.u.v4.sin_addr.s_addr)
		if(st->st_connection->spd.this.client_addr2.u.v4.sin_addr.s_addr == st_from->st_connection->spd.this.client_addr2.u.v4.sin_addr.s_addr)
		if(st->st_connection->spd.that.client_addr1.u.v4.sin_addr.s_addr == st_from->st_connection->spd.that.client_addr1.u.v4.sin_addr.s_addr)
		if(st->st_connection->spd.that.client_addr2.u.v4.sin_addr.s_addr == st_from->st_connection->spd.that.client_addr2.u.v4.sin_addr.s_addr)
//		if(st->st_connection->spd.that.client.v.range.from.u.v4.sin_addr.s_addr == st_from->st_connection->spd.that.client.v.range.from.u.v4.sin_addr.s_addr)
//		if(st->st_connection->spd.that.client.v.range.to.u.v4.sin_addr.s_addr == st_from->st_connection->spd.that.client.v.range.to.u.v4.sin_addr.s_addr)
		if(st->st_connection->host_pair->me.addr.u.v4.sin_addr.s_addr == st_from->st_connection->host_pair->me.addr.u.v4.sin_addr.s_addr)
		if(st->st_connection->host_pair->me.port == st_from->st_connection->host_pair->me.port)
		if(st->st_connection->host_pair->him.addr.u.v4.sin_addr.s_addr == st_from->st_connection->host_pair->him.addr.u.v4.sin_addr.s_addr)
		if(st->st_connection->host_pair->him.port == st_from->st_connection->host_pair->him.port)
		if(from != st)
#endif
		{
			return st;
...
/* Add by Daniel Cheng 2008.11.18 phase2 FSM -->
   PHASE2_INITIATOR from_type = 0
   PHASE2_RESPONDER from_type = 1
 */
int check_phase2_FSM(struct state *from , bool from_type)
{
    struct state *st, *st_from = from;
    bool st_from_type = from_type;
    int i, find_st = NULL;
	...
	for (st = statetable[i]; st != NULL; st = st->st_hashchain_next)
	{
		...
		if(st->st_state>=STATE_QUICK_R0 && st->st_state<=STATE_QUICK_R1 /*&& st!=st_from*/)
#if 0
		if(memcmp(&(st->st_connection->spd.this),&(st_from->st_connection->spd.this),sizeof(struct end)))
		if(memcmp(&(st->st_connection->spd.that),&(st_from->st_connection->spd.that),sizeof(struct end)))
		if(memcmp(st->st_connection->host_pair,st_from->st_connection->host_pair,sizeof(struct host_pair)))
#endif
#if 1
		if(st->st_connection->spd.this.client_addr1.u.v4.sin_addr.s_addr == st_from->st_connection->spd.this.client_addr1.u.v4.sin_addr.s_addr)
		if(st->st_connection->spd.this.client_addr2.u.v4.sin_addr.s_addr == st_from->st_connection->spd.this.client_addr2.u.v4.sin_addr.s_addr)
...
//		if(st->st_connection->spd.that.client.v.range.to.u.v4.sin_addr.s_addr == st_from->st_connection->spd.that.client.v.range.to.u.v4.sin_addr.s_addr)
	...
	if(st->st_connection->host_pair->me.addr.u.v4.sin_addr.s_addr < st->st_connection->host_pair->him.addr.u.v4.sin_addr.s_addr)
	{
		if(st_from_type == PHASE2_FSM_INITIATOR)
			return 1;
		else
		{
...
struct state *find_state_by_icookie(const u_char *icookie)
{
    int i;
    struct state *st;
    ...
    for (i = 0; i < STATE_TABLE_SIZE; i++)
    {
		for (st = statetable[i]; st != NULL; st = st->st_hashchain_next)
		{
			if(st->st_connection == NULL)
				continue;
			if(memcmp(st->st_icookie, icookie, COOKIE_SIZE) == 0)	
			{
				return st;
			}
		}
