 ...
 * for more details.
 *
 * RCSID $Id: whack.c 4962 2010-05-10 02:30:30Z charles.lin $
 */
...
/* Encounter: following four are just for alignment with POLICY order */
    CD_OPPO,	/* same order as POLICY_* */
...
#endif
    CD_NATTRAVERSAL,	/* same order as POLICY_* */
...
#   define OO	OPTION_OFFSET
...
#ifdef SUPPORT_IPRANGE    
    { "clientrange", required_argument, NULL, END_CLIENTRANGE + OO },
#endif
...
#endif
    { "nat_traversal", no_argument, NULL, CD_NATTRAVERSAL + OO },
	...
	{ "auth-alg", required_argument, NULL, CD_AUTHALG + OO },
...
main(int argc, char **argv)
{
   ...
   /* if a connection is added via whack then we assume IKEv1 */
	...
	case END_CLIENTWITHIN:	/* --clienwithin <address range> */
	    ...
	    continue;
#ifdef SUPPORT_IPRANGE
	case END_CLIENTRANGE:	/* --clientrange <address range> */
	    tunnel_af_used_by = long_opts[long_index].name;
	    {
		static char ipstring1[20];
		static char ipstring2[20];
		int i,j,k=0;
		for( i=0,j=0; i<20; i++ )
		{
		    if( optarg[i] == '.' || (optarg[i] >= '0' && optarg[i] <= '9') )
		    	ipstring1[j++] = optarg[i];
		    else
		    	break;
		}
		ipstring1[j]=0;
		k=i+1;
		for( i=k, j=0; i<k+20; i++ )
		{
		    if( optarg[i] == '.' || (optarg[i] >= '0' && optarg[i] <= '9') )
		    	ipstring2[j++] = optarg[i];
		    else
		    	break;
		}
		ipstring2[j]=0;
		ttoaddr(ipstring1, 0, msg.tunnel_addr_family, &msg.right.client_addr1);
		ttoaddr(ipstring2, 0, msg.tunnel_addr_family, &msg.right.client_addr2);
		if( msg.right.client_addr1.u.v4.sin_addr.s_addr ==
		    msg.right.client_addr2.u.v4.sin_addr.s_addr )
		{
	            msg.right.client_id = ID_IPV4_ADDR;
		    /* Freeswan always looks client as a subnet and verifies it. This bother me a lot.
		       So I filled the subnet data as below for cheating the freeswan */
		    strcat(ipstring1, "/32");
		    ttosubnet(ipstring1, 0, msg.tunnel_addr_family, &msg.right.client);
		}
		else
		{
	            msg.right.client_id = ID_IPV4_ADDR_RANGE;
		    /* Freeswan always looks client as a subnet and verifies it. This bother me a lot.
		       So I filled the subnet data as below for cheating the freeswan
		       Notice that the subnet used here must coincidant with one in 
		       ipsec_doi.c/decode_net_id() */

		    //rangetosubnet(&msg.right.client_addr1, &msg.right.client_addr2, &msg.right.client);
		    strcat(ipstring1, "/24");
		    ttosubnet(ipstring1, 0, msg.tunnel_addr_family, &msg.right.client);
		}
#ifdef NK_DEBUG_IDTYPE
		rg_error(LERR, "whack.c/main(): addr_1=[%s](0x%x)  addr_2=[%s](0x%x)",
		    ipstring1, msg.right.client_addr1.u.v4.sin_addr.s_addr,
		    ipstring2, msg.right.client_addr2.u.v4.sin_addr.s_addr);
		rg_error(LERR, "whack.c/main(): client subnet=[%x/%d]",
		    msg.right.client.addr.u.v4.sin_addr.s_addr,
		    msg.right.client.maskbits);
#endif
	    }
...
#if 1 //aggressive mode
        case CD_AGGRESSIVE:     /* --aggr */
#endif
       case CD_NATTRAVERSAL:   /* --nat_traversal */
...
#if 0  /* We have no such restriction */
...
*/	
	...
	|| !pack_str(&msg.auth_alg)		/*string addnew:support authenticate algorithem*/
    || !pack_str(&msg.esp)		/* string 23 */
    || !pack_str(&msg.sc_data)		/* string 24 */
    || !pack_str(&msg.pskey)	//20100126 trenchen : support preshare key get from whack ipc	
