...
#endif
{
	...
	static struct option m_options[] = {
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:fH:hi:np:qr:s:v", m_options, &option_index);
		if (c == -1) break;
	...
	if ((fd = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) >= 0) {
		...
		if (ioctl(fd, SIOCGIFINDEX, &ifr) == 0) {
			...
			LOG(LOG_INFO, "SIOCGIFINDEX failed! %s", strerror(errno));
			exit_client(1);
		}
		if (ioctl(fd, SIOCGIFHWADDR, &ifr) == 0) {
			...
			LOG(LOG_INFO, "SIOCGIFHWADDR failed! %s", strerror(errno));
			exit_client(1);
		}
	} else {
		LOG(LOG_INFO, "socket failed! %s", strerror(errno));
		exit_client(1);
	}
	close(fd);
	fd = -1;
	...
	for (;;) {
		...
		if (listen_mode == LISTEN_KERNEL) {
			...
			if ((fd = raw_socket(client_config.ifindex)) < 0) {
				LOG(LOG_INFO, "couldn't create raw socket -- au revoir");
				exit_client(0);
			}			
		} else fd = -1;
			...
			case INIT_SELECTING:
				...
				if (*message == DHCPOFFER) {
					if ((temp = get_option(&packet, DHCP_SERVER_ID))) {
						...
						DEBUG(LOG_INFO, "No server ID in message");
					}
				}
				break;
			case RENEW_REQUESTED:
			...
			case REBINDING:
				if (*message == DHCPACK) {
					if (!(temp = get_option(&packet, DHCP_LEASE_TIME))) {
						LOG(LOG_INFO, "No lease time with ACK, using 1 hour lease");
						lease = 60*60;
					} else {
						memcpy(&lease, temp, 4);
						lease = ntohl(lease);
