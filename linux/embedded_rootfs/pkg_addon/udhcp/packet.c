...
int get_packet(struct dhcpMessage *packet, int fd)
{
	...
	if (ntohl(packet->cookie) != DHCP_MAGIC) {
		LOG(LOG_INFO, "received bogus message -- ignoring");
		return -1;
	}
		   ...
		   u_int32_t dest_ip, int dest_port, char *dest_arp, int ifindex)
{
	...
	if ((fd = socket(PF_PACKET, SOCK_DGRAM, htons(ETH_P_IP))) < 0) {
		DEBUG(LOG_INFO, "socket call failed: %s", strerror(errno));
		return -1;
	}
	...
	if (bind(fd, (struct sockaddr *)&dest, sizeof(struct sockaddr_ll)) < 0) {
		DEBUG(LOG_INFO, "bind call failed: %s", strerror(errno));
		close(fd);
		return -1;
	}
	...
	if (result <= 0) {
		DEBUG(LOG_INFO, "write on socket failed: %s", strerror(errno));
	}
	close(fd);
	return result;
}
