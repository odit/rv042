...
//	return arpCheck(yiaddr, &ifbuf, 2);
	return arpCheck(yiaddr, &ifbuf, 1);
}
...
int arpCheck(u_long inaddr, struct ifinfo *ifbuf, long timeout)  {
	...
	struct timeval	tm;
	time_t			prevTime;
	struct timeval	nk_prev_tv;
	int nk_counter = 2;
	int nk_timer = timeout * nk_counter;
	rv = 1;
	...
	time(&prevTime);
	gettimeofday(&nk_prev_tv,NULL);
	timeout = timeout * nk_counter * 100000;//fix use usec and set timeout 0.1 sec
	// nk_timer set twice So time out =0.1 sec
	while ( nk_timer > 0 ) {
		FD_ZERO(&fdset);
		FD_SET(s, &fdset);
		tm.tv_sec  = 0;
		...
		if ( select(s+1, &fdset, (fd_set *)NULL, (fd_set *)NULL, &tm) < 0 ) {
			DEBUG(LOG_INFO, "Error on ARPING request: %s", strerror(errno));
			if (errno != EINTR) rv = 0;
...
int openRawSocket (int *s, u_short type) {
	...
	if((*s = socket (AF_INET, SOCK_PACKET, htons (type))) == -1) {
		LOG(LOG_INFO, "Could not open raw socket");
		return -1;
	}
	
	if(setsockopt (*s, SOL_SOCKET, SO_BROADCAST, &optval, sizeof (optval)) == -1) {
		LOG(LOG_INFO, "Could not setsocketopt on raw socket");
		return -1;
	}
	return 0;
}
