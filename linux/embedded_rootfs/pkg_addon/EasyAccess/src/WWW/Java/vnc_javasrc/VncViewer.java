...
public class VncViewer extends java.applet.Applet
  ...
  public void init() {
    ...
    if (inSeparateFrame) {
      vncFrame = new Frame("Virtual Network Connection");
      if (!inAnApplet) {
  ...
  public void readParameters() {
    if(inAnApplet && (getCodeBase() != null))
      tunnelServer = new String(getCodeBase().getHost());

	System.out.println(tunnelServer);
	
    if((tunnelServer == null) || (tunnelServer.equals("")))
        tunnelServer = new String(readParameter("TUNNEL", true));

    tunnelSession = new String(readParameter("SESSION", true));

    if(readParameter("TIMEOUT", true) != null)
     {
        Integer tOut = new Integer(readParameter("TIMEOUT", true));
         timeOut = tOut.intValue();
     }
     else
        timeOut = 0;
		
	if(readParameter("HTTPS_PORT", true) != null)
     {
        Integer tOut = new Integer(readParameter("HTTPS_PORT", true));
         HTTPS_PORT = tOut.intValue();
     }
     else
        HTTPS_PORT = 443;
