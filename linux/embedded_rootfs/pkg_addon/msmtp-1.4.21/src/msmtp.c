...
#endif
#include <nkutil.h>
void sendResultToSyslogd(int result, char *email);
...
exit:
    sendResultToSyslogd(error_code,account->from);
    /* clean up */
    if (tmpfile)
    {
        fclose(tmpfile);
...
/**
 * purpose : CONFIG_IPV6
 * description : unified interface to send system log
 * @param result: EX_OK successful, others failed
 * @param email: email address set in UI
**/
void sendResultToSyslogd(int result, char *email)
{
	if(result == EX_OK)
	{
		NK_LOG_SYS(LOG_WARNING, "Sending email to %s is successful !\n", email);
	}
	else
	{
		NK_LOG_SYS(LOG_WARNING, "Sending email to %s is failed !\n", email);
	}
}
