...
oid             nk_sysobjectid_042[] = {1, 3, 6, 1, 4, 1, 9, 6, 1, 11, 42, 1};
oid             nk_sysobjectid_082[] = {1, 3, 6, 1, 4, 1, 9, 6, 1, 11, 82, 1};
oid             nk_sysobjectid_016[] = {1, 3, 6, 1, 4, 1, 9, 6, 1, 11, 16, 1};
int             nk_sysobjectid_len = OID_LENGTH(nk_sysobjectid_042);
#else
oid             nk_sysobjectid[] = { 1, 3, 6, 1, 4, 1, 23871};
int             nk_sysobjectid_len = OID_LENGTH(nk_sysobjectid);
#endif
...
void system_parse_config_sysObjectID(const char *token, char *cptr)
{
    ...
    if (!read_objid(cptr, sysObjectID, &sysObjectIDLength)) {
        ...
        //sysObjectIDLength = version_sysoid_len;
				...
				if (!strcmp("RV042", tmpbuf))
				{
					memcpy(sysObjectID, nk_sysobjectid_042, nk_sysobjectid_len * sizeof(oid));
				}
				if (!strcmp("RV082", tmpbuf))
				{
					memcpy(sysObjectID, nk_sysobjectid_082, nk_sysobjectid_len * sizeof(oid));
				}
				if (!strcmp("RV016", tmpbuf))
				{
					memcpy(sysObjectID, nk_sysobjectid_016, nk_sysobjectid_len * sizeof(oid));
				}	
#else
...
system_store(int a, int b, void *c, void *d)
{
    ...
    if (sysLocationSet > 0) {
        ...
        snmpd_store_config(line);
				...
				sprintf(line, "SNMP SLOCATION=%s", sysLocation);
				kd_doCommand(line, CMD_WRITE, ASH_DO_NOTHING, (char *) NULL);
    ...
    if (sysContactSet > 0) {
        ...
        snmpd_store_config(line);
				// Added by Gavin Lin, 2010/04/16
				sprintf(line, "SNMP SCONTACT=%s", sysContact);
				kd_doCommand(line, CMD_WRITE, ASH_DO_NOTHING, (char *) NULL);
				kd_updateFlash(USER_CHANGE_DB);
    }
    if (sysNameSet > 0) {
        ...
        snmpd_store_config(line);
				// Added by Gavin Lin, 2010/04/16
				sprintf(line, "SNMP SNAME=%s", sysName);
				kd_doCommand(line, CMD_WRITE, ASH_DO_NOTHING, (char *) NULL);
				kd_updateFlash(USER_CHANGE_DB);
    }
