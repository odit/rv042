 ...
 *
 * $Id: cvmx-dfa.h 2 2007-04-05 08:51:12Z tt $ $Name$
 */
...
/**
 * These structures defines a DFA instruction
 */
typedef union
{
    uint64_t u64[4];
    ...
    {
        // WORD 0
        uint64_t unused0                : 12;   /**< Must be zero */
        uint64_t start_node             : 20;   /**< Node to start the walk from */
...
/**
 * Format of the second result word written by the hardware.
 */
typedef union
{
    uint64_t u64;
...
typedef struct
{
    ...
    int                         start_node;         /**< Start node index for the root of the graph */
    ...
    *head++ = *command;
    ...
    if (cvmx_unlikely((++index >= ((CVMX_FPA_DFA_POOL_SIZE-8)/sizeof(cvmx_dfa_command_t))))) {
	    ...
	    // update our state (note 32-bit write to not disturb other fields)
            ...
            cvmx_dprintf("cvmx_dfa_write_command: Out of memory. Expect crashes.\n");
        }
	index=0;
    }
                                  ...
                                  cvmx_dfa_result0_t *result, int max_results, cvmx_wqe_t *work)
{
    ...
    // WORD 0
    command.u64[0] = 0;
    command.s.start_node    = start_node;
    ...
    cvmx_dfa_write_command(&command);	// NOTE: this does synchronization and rings doorbell
}
#endif
