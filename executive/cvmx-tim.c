 ...
 *
 * File version info: $Id: cvmx-tim.c 2 2007-04-05 08:51:12Z tt $
 */
    ...
    for (timer_id = 0; timer_id<CVMX_TIM_NUM_TIMERS; timer_id++)
    {
        for (bucket=0; bucket<cvmx_tim.num_buckets; bucket++)
        {
            ...
            while (bucket_ptr->num_entries)
            {
                ...
                next_chunk_addr = cvmx_read64(CVMX_ADD_SEG(CVMX_MIPS_SPACE_XKPHYS, chunk_addr + CVMX_FPA_TIMER_POOL_SIZE - 8));
