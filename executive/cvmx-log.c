 ...
 *
 * $Id: cvmx-log.c 2 2007-04-05 08:51:12Z tt $ $Name$
 */
 ...
 */
static void cvmx_log_initialize(void) CVMX_LOG_DISABLE_PC_LOGGING;
static void cvmx_log_initialize(void)
{
    int buf_num;
 ...
 */
static void cvmx_log_full_process(void) CVMX_LOG_DISABLE_PC_LOGGING;
static void cvmx_log_full_process(void)
{
 ...
 * @return The header
 */
static inline uint64_t cvmx_log_build_header(cvmx_log_type_t type, uint64_t size) CVMX_LOG_DISABLE_PC_LOGGING;
static inline uint64_t cvmx_log_build_header(cvmx_log_type_t type, uint64_t size)
{
 ...
 */
static inline void cvmx_log_write(uint64_t data) CVMX_LOG_DISABLE_PC_LOGGING;
static inline void cvmx_log_write(uint64_t data)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (cvmx_likely(cvmx_log_buffer_write_ptr))
            {
                ...
                cvmx_log_full_process();    /* After this function returns, the log must be ready for updates */
        }
        *cvmx_log_buffer_write_ptr++ = data;
    }
}
...
{
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PC, 1));
    cvmx_log_write(pc);
}
...
{
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PRINTF, 1));
    cvmx_log_write(CAST64(format));
}
...
{
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PRINTF, 2));
    cvmx_log_write(CAST64(format));
    cvmx_log_write(number1);
}
...
{
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PRINTF, 3));
    cvmx_log_write(CAST64(format));
    cvmx_log_write(number1);
    cvmx_log_write(number2);
}
...
{
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PRINTF, 4));
    cvmx_log_write(CAST64(format));
    cvmx_log_write(number1);
    cvmx_log_write(number2);
    cvmx_log_write(number3);
}
...
{
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PRINTF, 5));
    cvmx_log_write(CAST64(format));
    cvmx_log_write(number1);
    cvmx_log_write(number2);
    cvmx_log_write(number3);
    cvmx_log_write(number4);
}
...
void cvmx_log_data(uint64_t size_in_dwords, const uint64_t *data)
{
    ...
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_DATA, size_in_dwords));
...
void cvmx_log_structure(const char *type, void *data, int size_in_bytes)
{
    ...
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_STRUCTURE, size_in_dwords + 1));
    cvmx_log_write(CAST64(type));
    while (size_in_dwords--)
        cvmx_log_write(*ptr++);
}
...
void cvmx_log_perf(void)
{
    ...
    asm ("mfc0 %0, $25, 0\n" : "=r"(control1));
    asm ("mfc0 %0, $25, 2\n" : "=r"(control2));
    cvmx_log_write(cvmx_log_build_header(CVMX_LOG_TYPE_PERF, 3));
    cvmx_log_write(((control1 & 0xffffffff) << 32) | (control2 & 0xffffffff));
    cvmx_log_write(data1);
    cvmx_log_write(data2);
}
 ...
 * @return the dword
 */
static uint64_t cvmx_log_read(void) CVMX_LOG_DISABLE_PC_LOGGING;
static uint64_t cvmx_log_read(void)
{
    uint64_t data;
    ...
    while (cvmx_log_buffer_read_ptr && (cvmx_log_buffer_read_ptr != cvmx_log_buffer_write_ptr))
    {
        ...
        switch (header.s.type)
        {
            case CVMX_LOG_TYPE_PC:
                if (header.s.size == 1)
                    printf("pc 0x%016llx\n", (unsigned long long)cvmx_log_read());
                else
            ...
            case CVMX_LOG_TYPE_PRINTF:
                ...
                {
                    case 1:
                        printf(CASTPTR(const char, cvmx_log_read()));
                        break;
                    case 2:
                        printf(CASTPTR(const char, cvmx_log_read()), cvmx_log_read());
                        break;
                    case 3:
                        printf(CASTPTR(const char, cvmx_log_read()), cvmx_log_read(), cvmx_log_read());
                        break;
                    case 4:
                        printf(CASTPTR(const char, cvmx_log_read()), cvmx_log_read(), cvmx_log_read(), cvmx_log_read());
                        break;
                    case 5:
                        printf(CASTPTR(const char, cvmx_log_read()), cvmx_log_read(), cvmx_log_read(), cvmx_log_read(), cvmx_log_read());
                        break;
                    default:
            ...
            case CVMX_LOG_TYPE_DATA:
                ...
                for (i=0; i<header.s.size; i++)
                    printf(" 0x%016llx", (unsigned long long)cvmx_log_read());
                printf("\n");
                break;
            case CVMX_LOG_TYPE_STRUCTURE:
                printf("struct %s", CASTPTR(const char, cvmx_log_read()));
            ...
            case CVMX_LOG_TYPE_PERF:
                if (header.s.size == 3)
                {
                    unsigned long long control = cvmx_log_read();
                    unsigned long long data1 = cvmx_log_read();
                    unsigned long long data2 = cvmx_log_read();
